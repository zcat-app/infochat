 ...
 * DEMOTE_GROUP_ADMIN_INTENT row on a separate auto-commit connection
 * (spec §Authorization model step 8 "Audit-log the intent" precedes
 * step 9 "Execute"; GRANT_ADMIN_INTENT placement) — so the
 * unknown-contact and target-not-admin refusal legs leave a surviving
 * intent row while non-admin-caller refusals stay audit-silent. The
 * pre-transaction placement avoids the FK-vs-FOR-UPDATE deadlock: the
...
public class DemoteCommandHandler implements CommandHandler {
    ...
    public OutboundMessage handle(ScopeRef scope, String rawText) {
        ...
        // unknown-contact and target-not-admin refusal legs leave a
        // surviving intent row. The shared requestId correlates it
        // with the DEMOTE_GROUP_ADMIN effect row.
    ...
    // the admin gate and the membership UPDATE below (mirrors the
    ...
    /**
     * Write the DEMOTE_GROUP_ADMIN_INTENT row on its own auto-commit
    ...
    private void insertIntentAudit(String adapter, String targetContactId,
        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            throw new IllegalStateException(
                    "DemoteCommandHandler intent-audit write failed for adapter="
