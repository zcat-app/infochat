...
- Comments and javadoc state current truth, briefly; javadoc states the contract. Neither retells history — no chronicle of tickets, audits, or how the code came to be. Detail is referenced with one stable pointer (spec-section anchor, decision ID, ticket ID, or `docs/plan/...` path), never summarized. Caller/usage references ("used by X", "added for the Y flow") rot and stay out. The pointer forms rank by rot-resistance: a spec/design anchor points at maintained current truth, a decision ID at a still-binding choice, a ticket ID at a frozen premise later work can falsify without notice — so a ticket ID in a code comment carries business weight (an §8 authorization) or resolves a ticket-internal reference; pure provenance stays in the commit message.
...
### The spec changes by approval and states rules only
- Any edit under `docs/spec/**` is shown to the user first — exact proposed text plus plain-English reasoning about the commitment change — and lands only after an explicit yes. A ticket whose acceptance lists an amendment authorizes the work; the user still approves the wording.
- Spec prose states what the system promises, nothing else: no dates, ticket IDs, or redteam/audit/live-test report citations. History belongs in the decision register, the ticket, or the analysis document. Older spec text carries such citations; they are legacy, not license.
- Verbatim text in [`engineering-rules-verbatim.md`](docs/process/engineering-rules-verbatim.md) §12; the tick reviewer checks the style leg and the recorded approval.
...
Output from **either** → run `./mvnw -B -pl infochat-provider -am test` green before landing (~2 min; surefire only, no `*IT` suite). Both silent → the zero-verify bypass stands. The second probe is separate because the first cannot see a **rename or delete** of the two single-file fixtures — git emits `rename from`/`rename to` (no `+++ b/` at all) for a pure rename and `+++ /dev/null` for a delete, yet both break the three tests that locate those files by literal path; asking git directly which paths were renamed-away or deleted is what keeps that leg from being widened into "any touch of `docs/spec/**`" (M1-773). Only ADDED tokens can fail the scrape (documented ⊆ real ∪ exempt; the vacuity floors are 50/50, far below the repo's counts). A failure is fixed by correcting the doc or adding a ledger entry — never by narrowing the scan. Full rule + rationale: `docs/process/workflow.md` §Non-ticket commits rule 10.
