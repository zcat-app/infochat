...
- **Round 2 must shrink.** Round-2 diff must be smaller than round-1 along **at least one** of: files-touched, lines added, lines removed. Growth along **all three** dimensions simultaneously → automatic SCOPE-DRIFT-CHECK fail unless round-1 REWORK explicitly required a refactor that grows the diff (citation required).
