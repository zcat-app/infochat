...
/**
 * Single-instance enforcement guard for the Collector service (decision D41).
 *
 * <p>Acquires {@code pg_try_advisory_lock(hashtext('infochat.collector'))} at
 * {@link Startup} priority 50 ({@code docs/design/01-architecture.md} §1.4.3).
 * The hash is computed server-side by Postgres' built-in {@code hashtext},
 * so two instances on different hosts always race for the same lock id with
 * no client-side hashing routine.
 *
 ...
 * Collector could quietly take over while this JVM was still alive. That is
 * why {@link #heldConnection} is borrowed from the datasource and never
 * returned to the pool until shutdown.
 *
...
public class InstanceLockGuard {
    ...
    static final String SERVICE = "collector";
    ...
    /**
     * Calls {@code pg_try_advisory_lock(hashtext('infochat.collector'))} on
