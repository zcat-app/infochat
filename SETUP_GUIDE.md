...
| **Bot runs, but free-text help is dead — it only offers `/help`, no topic answers** | The embeddings model (the part that matches by meaning) isn't serving. This is a safe degraded mode — commands, news, digests and every security control keep working. Recognition signals: the Provider's readiness payload (`/q/health/ready`) carries a `help-corpora` check whose data marks the failed corpus `false` (the check itself stays UP — it reports the boot-time build outcome, not backend liveness), and step 8's verify prints a WARN line naming the embedding backend. Check the backend service: `docker compose --profile ollama ps` (or `--profile llamacpp-embeddings ps` if embeddings run on llama.cpp); if it's down, start it, confirm the embeddings model is present (`docker compose --profile ollama exec ollama ollama list` should list `nomic-embed-text`), then restart the apps (`./prod/scripts/apps.sh restart`) — the help corpora rebuild at startup. |
