...
| [anchor-leg-characterization.md](anchor-leg-characterization.md) | Where the cross-lingual anchor leg loses: sibling-pair probe + three-arm counterfactual over every active xling row at the frozen pin. Answer: divergence concentrates in cyber (anchor −0.5 vs sibling) while crypto gains (+0.4); the dominant share is embedding-space phrasing sensitivity — the space is loose, the anchor not uniquely broken — and embedder-server restarts drift bounded (≤0.0023, adjacent near-tie reorders only). All smoke at n = 12: decision-informing, never gating. |
