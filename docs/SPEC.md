...
| Command UX | Slash-prefix only; single `-w` time flag (1h/24h/7d/1m); friendly errors with fuzzy match; `<command> confirm` for destructive | [03](03-commands.md) |
...
| Eval failure policy | **Per-stage handling.** Security stage: 1 retry → quarantine (never fall through). Tagger stage: 1 retry → fallback to source's bootstrap tags → admin notify. Embedding stage: 1 retry → release post without embedding (skip Tier 2 linking). Admin notifications throttled: 1 per failure-class per 15 min, with count. | [04](04-security.md), [05](05-llm-and-embeddings.md) |
...
- **Topic ID**: ID of a post cluster (connected component in `post_reference`). Stable for the lifetime of the cluster.
