...
**Lifecycle-path exemption.** Two paths are workflow byproducts produced
by the `/m1-tick` skill itself, NOT developer choices, and therefore are
EXEMPT from both the numeric-budget check and the `files_scope`
membership check:
...
When applying the budget and scope checks: subtract both paths from
...
is evaluated as if only `<implementation-files>` were present.

The exemption is one-directional: the two paths are forgiven, never
...
list the two paths in `files_scope` and a correspondingly inflated
