...
**Lifecycle-path exemption.** Four paths are workflow byproducts produced
by the `/m1-tick`, `/redteam`, and `/redteam-multi` skills themselves,
NOT developer choices, and therefore are EXEMPT from both the
numeric-budget check and the `files_scope` membership check:
  ...
  - The multi-auditor evidence directory at
    `docs/plan/m1/redteam-multi/{{TICKET_ID}}-*{{DATE}}[-rN]/` — written
    by `/redteam-multi {{TICKET_ID}}` per
    `.agents/skills/redteam-multi/SKILL.md` step 6 and single-sourced in
    `docs/process/harness-mapping.md` §7. Contains the per-auditor
    prompts/verdicts/replies, the cross-examination report, and the
    contamination-check porcelain snapshots. Folded into the ticket
    commit when the audit ran on that ticket's branch (the default for
    `security_relevant: true` tickets, which run the multi-auditor gate
    before review). The same one-directional exemption rule applies:
    lifecycle-driven changes are forgiven; arbitrary edits to the
    directory's contents are not.

When applying the budget and scope checks: subtract all four paths from
...
Equivalently: a diff of `<implementation-files> + STATUS.md +
ticket-file + redteam-verdict-file + redteam-multi-dir` is evaluated as
if only `<implementation-files>` were present.

The exemption is one-directional: the four paths are forgiven, never
