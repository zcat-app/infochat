...
| `spec:` | Pure spec/design edit under `docs/spec/` or `docs/design/`, no code change | ticket-readiness pre-flight, reviewer, STATUS regen, `mvn verify` — but run the rule 10 probes first; some doc content is test input |
| `process:` | Edit under `.claude/`, `docs/process/`, `docs/plan/`, `CLAUDE.md`, or the agent-tooling surface (`AGENTS.md`, `.agents/`, `.opencode/`, `.codex/`, `CONTRIBUTING.md`), no code change | ticket-readiness pre-flight, reviewer, STATUS regen, `mvn verify` — but run the rule 10 probes first if the edit touches a root-level `*.md` |
...
10. **Parity-gated doc content — some docs ARE test input.** The `mvn verify` bypass in the table above rests on "a pure-doc edit cannot break the build". For some doc *content* that premise is simply false, because four tests read doc files as fixtures:
   ...
   **The trigger is what the diff CONTAINS, not which directory it sits in.** Gating on the directory would charge the gates to nearly every `docs/spec/**`, `docs/design/**` and root-guide edit, and the overwhelming majority of those cannot fail one — which would re-create, at a smaller scale, the too-expensive-to-follow rule this is replacing. Stage the change, then let these two probes decide:
   ...
   git diff --cached --name-only --diff-filter=RD -- docs/spec/commands.md docs/spec/security.md
   ```

   **Output from either → run the gates. Both silent → the bypass stands, commit as normal.** How the first probe maps onto the four tests: the `:(top,glob)` pathspec admits root-level `*.md` only (under `:(glob)` a `*` does not cross `/`), so `docs/process/**`, `docs/plan/**`, `.claude/**` and `.agents/**` never enter the diff at all; the first alternation catches a *content* touch of either single-file fixture; the second catches an ADDED line naming a config key. In that second arm the group is **optional** so a token at column 0 still matches — `+infochat.foo=bar`, the properties-block style a design note documenting a new key most naturally uses — while `[^+]` inside it keeps the `+++ b/…` file header out (the header's second character is `+`, so the group cannot match, and no bare `infochat.` follows the first `+`).

   **Why the second probe is a separate command, and not a wider regex** (M1-773). The first probe detects the two single-file fixtures only through a `+++ b/<path>` hunk header, and git does not emit one for every change to a file: a pure rename produces `similarity index` / `rename from` / `rename to` with no `---`/`+++` pair at all, and a delete produces `+++ /dev/null`. Either would leave the probe silent while genuinely breaking `CommandCatalogueParityTest`, `LlmOutputSanitizerTest.matchSetEqualsSpecClosedList()` and `ChatToolAllowlistSpecParityTest`, all three of which locate their fixture by literal path — the same red `main` as `835ab637`, reached through a different door. Widening the *regex* to also match `--- a/…` and `rename from …` was tried and rejected: it still misses the delete of an **empty** fixture, for which git emits `deleted file mode` and an index line and nothing else. Asking git which paths were renamed-away or deleted (`--diff-filter=RD`, pathspec-limited to exactly the two files) is a question about the change rather than about the diff's text formatting, so it does not depend on which artefacts the format happens to produce. Do NOT "simplify" this back into the first regex, and do NOT relax the pathspec to `docs/spec` — a directory-level probe would flag every rename anywhere under the spec tree, re-creating the over-charging the content-keyed design exists to avoid. `DocumentedConfigKeyParityTest` needs no equivalent leg: it walks whole trees instead of naming files, so a deleted doc removes documented keys rather than breaking a lookup, and its vacuity floors sit far below the repo's counts.
