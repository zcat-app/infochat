...
2. Use the Read tool to load each ticket file from the Glob result. Parse the YAML frontmatter between the opening `---` and closing `---`. The keys you need: `id`, `title`, `status`, `blocked_by` (list), `deferred_on` (list, optional), `deferred_reason` (optional), `complexity`, `risk`, `last_updated`, and the most recent entry under `reviews:` (used for the in-flight and done tables).
