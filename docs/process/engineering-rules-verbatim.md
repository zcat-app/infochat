...
- A comment states current truth, briefly. Javadoc states the contract. Neither retells history — no chronicle of tickets, audits, or how the code came to be. Detail lives in the spec, the design or analysis document, the ticket, or the decisions log; the comment points at it with ONE stable reference — a spec-section anchor, a decision ID (D-NN), a ticket ID (M1-NNN), or a `docs/plan/...` path — never a caller/usage reference ("used by X", "added for the Y flow"), which rots. (This preserves M1-311's carve-out: the stable anchor is permitted AS the pointer; what is forbidden is the chronicle.)
- The pointer forms differ in rot-resistance: a spec/design anchor points at MAINTAINED current truth (the spec is amended when behavior changes), a decision ID at a still-binding choice, but a ticket ID at a FROZEN premise — later work can falsify the ticket's assumptions without notice, and the anchor then silently points at a stale assumption. A ticket ID therefore belongs in a code comment only where it carries business weight (an §8 test-modification authorization is the canonical case) or resolves an otherwise-dangling ticket-internal reference (a pitfall number is unresolvable without its ticket ID); a pure provenance tag has no place in a comment — provenance lives in the commit message, where the `M<N>-NNN:` prefix makes git-blame → ticket immutable.
...
## §13 Deployment-identifying material never commits

- A committed artifact names no deployment: no container, host, network, volume, or project names of a real instance; no live or replica ports or connection topologies; no DB fingerprints, ready counts, or state pins of a non-fixture instance; no aggregate counts derived from real users' rows (scope-language splits, census tallies); no dumps, run records, or pin readouts. A fact is deployment-identifying if it lets a repo reader distinguish one live machine from another.
- Procedures and fixtures may commit when they are instance-free — generic mechanics, placeholder or generated identifiers, and content-addressed hashes of public corpus material. Anything bound to a particular instance lives in the gitignored operator-local stores (`.agents/memory-local/`, `.bench/`); the committed artifact may state that such a local procedure exists and where its class of artifact lives, without naming the instance.
- Placement is decided at analysis time: the analyst classifies every artifact the plan will produce as committed or operator-local, and a ticket whose files-to-touch plan routes deployment-identifying material into the repo is a draft-time failure. The reviewer treats a committed deployment identifier in the diff as a FAIL of the placement leg — and one found after the fact is a history-rewrite event, not a follow-up ticket: once pushed, it cannot be recalled by an ordinary commit.
...
When this file changes, no re-render is needed — the reviewer reads this file directly at runtime. Keep the `CLAUDE.md` summary, the `docs/process/workflow.md` §Round-N must-shrink paragraph, the m1-tick `SKILL.md` must-shrink bullet, and the reviewer prompt's brief must-shrink restatement (verdict-format section) in sync by hand; there is no automated check. §11/§12 additionally propagate to `docs/process/tick-workflow.md` (§Principles 8 + the standing rules), `docs/process/tick-reviewer-prompt.md` (SPEC-TRUTHNESS style leg, MAINTAINABILITY comments), `docs/process/analyst-prompt.md` (spec-first rule), `.agents/skills/tick/subcommands/start.md` and `review.md`, and the deep-review prompts (`docs/process/deep-review-prompt-diff.md`, `-module.md`). §13 propagates to `docs/process/tick-workflow.md` (analyst produces, item 3), `docs/process/analyst-prompt.md` (placement classification), and `docs/process/tick-reviewer-prompt.md` (SCOPE-CHECK placement leg).
