...
- `infochat-core` MUST NOT depend on `infochat-messaging-adapter`. Enforced by a `maven-enforcer-plugin` `bannedDependencies` rule in `infochat-core/pom.xml` (M1-702), mirroring the Collector rule above. This is what makes core's "no `messaging-adapter` dependency" promise in the table structural rather than conventional — and with it, the module-DAG bound that `OutboundChokepointArchTest`'s provider-module scope rests on (`docs/spec/security.md` §"The chokepoint routing is build-guarded").
