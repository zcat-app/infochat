...
- Output JSON: {"tags": ["tag1","tag2"], "search_tags": ["free-tag-1"]}.                                                                                                                                             
- Tags must match the vocabulary EXACTLY (case-insensitive).                                                                                                                                                                                          
- search_tags is an optional second field of free retrieval tags outside the vocabulary (English; lower-case; single hyphenated words; no spaces; omit or [] when none fit).
- If none fit well, output {"tags": []}.                                                                                                                                                                                                              
- Never invent new tags (vocabulary categories — free retrieval tags in search_tags are the deliberate exception).
...
**Free-tag emission (search_tags).** The `search_tags` second field is a best-effort passenger of the same single call — zero additional LLM cost. Free tags are deliberately NOT vocabulary-checked: each entry is normalized through the shared tag pipeline (`TagNormalizer`: NFC + `Locale.ROOT` lower-case + character class) and entries failing that form are dropped, counted, and logged on the `tagger_partial_valid` line's pattern; duplicates after normalization collapse; at most `MAX_SEARCH_TAGS_PER_POST` (8 — 2× the design-intended 1–4, mirroring the `MAX_TAGS_PER_POST` Tier-1 cap) distinct valid free tags persist, in emission order. A missing, null, or non-array field stores `'{}'`; the field never triggers a retry or the bootstrap fallback; the line-oriented fallback prompt and the bootstrap path emit none. Survivors persist into `post.search_tags` (V87) through the SAME atomic cursor UPDATE as the Tier-1 tags — retrieval-only, never a digest/bootstrap/tag-tree surface.
...
**Tag tree and deterministic resolution (M1-865, decision 2).** The `tag` table carries the v2 tree shape (V82): `node_kind` (`'top' | 'leaf'`, DEFAULT `'leaf'`) and `parent_name` referencing `tag(name)`. `TagVocabulary` loads LEAVES ONLY — tops and the priority order never enter the prompt render, so the model proposes leaf names and nothing else — and publishes the full row set as a tree snapshot alongside. After `validate()` (normalization, membership, the `MAX_TAGS_PER_POST` cap on the pre-resolution list — unchanged), `TagTreeResolver` resolves the validated set to exactly ONE branch in pure Java: group by branch root, the winning branch is the one whose top ranks first in the fixed order Sport > Health > Fashion > Culture > Science > Tech > Business > News-last (a top outside the order — bounded Others — ranks below News); within the branch the deepest leaf wins, non-fallback leaves outrank fallback-marked leaves at equal depth, and equal-ranked ties keep emission order. The within-top sub-order is depth → non-fallback-before-fallback → emission order, the fallback designation arriving with the V84 seed (M1-866 via M1-878). News-last is the deterministic statement of "a news report that fits nothing else files under News->continent". Until the v2 seed lands (M1-866), every row is a parentless leaf, a parentless leaf is its own identity branch, and resolution is the identity — stored sets are byte-identical to the flat vocabulary. The single resolved leaf is written by the SAME atomic cursor UPDATE as `tagger_done`/`tagger_fallback`; the losing leaves are exposed on the outcome record and dropped until the Tier-2 candidate array lands (M1-868).

**Misc-share monitor (decision 5).** With `misc` a real leaf under the Others top, its share of tagger completions is the vocabulary-growth trigger: `MiscShareMonitor` mirrors `NoTagsRateMonitor` (sliding window, minimum sample, strict exceed, notifier coalescing as throttle, cold-start silence) and fires under the distinct error class `tagger.sustained_misc_share` — never `tagger.sustained_no_tags`, never `tagger.fallback_to_bootstrap`, because a rising misc share means the leaf list no longer covers the content mix, a different runbook from a dead tagger or per-post failures. A bootstrap-fallback completion counts as not-misc. Parameters (config keys, defaults shown):

| key | default | meaning |
|---|---|---|
| `infochat.tagger.misc-share-window-size` | 50 | sliding-window capacity in completions |
| `infochat.tagger.misc-share-min-sample` | 20 | minimum window contents before the share is evaluated |
| `infochat.tagger.misc-share-threshold` | 0.10 | misc share that must be strictly exceeded to alert |

**Re-evaluation sweep (M1-736).** An empty `tags` or `search_tags` array is terminal only for the inputs that produced it, and the inputs drift: the vocabulary grows (the `TagVocabulary` refresh path above) and the configured tagger model changes. After the live batch each tick, `TaggerWorker` re-runs the SAME chain (prompt, validation, atomic cursor write, failure ladder) over posts with `(tags='{}' OR search_tags='{}') AND tagger_done=TRUE AND tagger_fallback=FALSE` that have not yet been swept for the current input generation. What triggers a generation bump: a SHA-256 fingerprint over the sorted normalized vocabulary names, the `infochat.llm.tagger.model` string, and the loaded prompt templates (primary then fallback, in that fixed order — a prompt edit is an input change the sweep must see), compared against the singleton `tagger_sweep_state` row on every sweep-capable tick — a mismatch bumps `generation` by 1. The baseline fingerprint is recorded as generation 0 on first use and existing rows default `tagger_swept_generation=0`, so a deploy alone never triggers a backlog sweep; only the first real input change does. An operator swapping what answers behind the same endpoint URL is not detectable and is deliberately out of scope. Caps (config keys, defaults shown):
...
preserved. The strip then re-scans its own output to a fixpoint
(M1-875), retained as a bounded backstop: passes repeat while a pass
removes anything, up to a fixed pass cap; a bare opener kept as prose
carries its ruling across passes. Every balanced or exact span removal
is replaced by an elision separator (U+2026) that is neither whitespace
nor a letter, digit or hyphen, so a deletion never joins its neighbors
into a fresh token; a strip output holding nothing outside whitespace
