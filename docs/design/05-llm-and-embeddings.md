...
**v1 status — embeddings rows.** The `infochat.embeddings.model` and `infochat.embeddings.index-type` rows below show the *intended* per-profile design. v1 ships 768-d `nomic-embed-text` with an HNSW index on **every** profile (see §5.5); the per-profile embedding model / dimension / index are deferred beyond v1, and `infochat.embeddings.allow-model-change=false` keeps the dimension fixed.
