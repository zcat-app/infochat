...
6. **Reconcile the runtime file.** Bring the deployment's runtime `bootstrap-sources.json` (the compose mount at `/app/bootstrap-sources.json`) to tree-leaf names — hand-edit per the rulings (map or drop each name the preflight flagged on the file surface, writing the normalized lower-cased leaf forms), or delete the file and re-run `5-bootstrap.sh` (the wizard never clobbers an existing runtime file, `5-bootstrap.sh:137-138`). Preflight must now print `preflight: clean`.
