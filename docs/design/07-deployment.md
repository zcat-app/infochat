...
8. **Postflight.** `tag-tree-cutover.sh postflight` — GREEN on: history version 84 applied, 9 tops + 53 leaves, the 8 fallback-marked leaves, zero `nostr`/`video` anywhere, every `post.tags` / `source.bootstrap_tags` element naming a tag node, zero `scope_tag` orphans, and every runtime-file tag naming a node.
