...
# weighted sum of five integer-percentile terms — corroboration, reposts,
# likes, comments (the reddit reply count), source scarcity — gated by the
# urgent classification, tie-broken by recency. The denominator is the sum
# of the weights of the terms PRESENT on a cluster (NULL social columns
# drop out, so an editorial cluster is not structurally beaten by a social
# one). Hand-chosen and uncalibrated: tune against the live corpus by
# reading the per-term components ClusterProminence returns
# (docs/design/03-commands.md §3.12). Retuning is a config edit, not a
# code change; no fitting, no per-deployment variation.
infochat.digest.weight.corroboration=7
infochat.digest.weight.reposts=2
infochat.digest.weight.likes=1
infochat.digest.weight.comments=1
...
| `INFOCHAT_LLAMACPP_SPEC_TYPE` / `INFOCHAT_LLAMACPP_SPEC_DRAFT_GGUF` / `INFOCHAT_LLAMACPP_SPEC_N_MAX` | optional; llama.cpp only | `docker-compose.yml` | Speculative-decoding type, draft-head filename in the shared model volume, and draft depth. Defaults are `none`, empty, and `4`; the draft path is `/models/<filename>` when a filename is supplied. Ollama has no equivalent knobs, and the feature is off by default. |
...
| `prod/scripts/stack.sh {start\|stop\|restart\|status}` | `docker compose` `stop` / `start` / `ps` over the full assembly — base + comfyui overlay files, `--env-file` the runtime `secrets.env` when present, and all four profiles (`prod`, `ollama`, `llamacpp`, `llamacpp-embeddings`). | Full-stack lifecycle around a host reboot — every service the wizard can start, comfyui included; complements `apps.sh`'s app-only scope. `stop` preserves containers and data volumes (it is **not** a `down`); `start` resumes the existing container set (never `up`, so nothing is created or recreated) and fails with a `setup.sh` pointer when the stack was never created. The gpu overlay is never merged — it stays operator-manual. |
...
| 4 | `4-llm.sh` | Branch on the choice. **Ollama:** start the ollama service and `ollama pull` the profile's chat / security / embedding models. **llama.cpp (D49):** fetch the chosen generative GGUF (curated checksum-pinned default or operator override — a download URL or an absolute local path staged into the model volume), mint its filename plus its download URL + SHA into `secrets.env` (the URL/SHA let a host clone recover a *custom* GGUF on restore; a **staged local path persists an EMPTY URL** — it is not re-fetchable on a fresh host, which the wizard discloses at setup and restore.sh fails loud on, with the manual-staging recipe), and start the `llamacpp` service (`LLAMA_ARG_MODEL`/`LLAMA_ARG_HOST`), probing `/dev/dri` render nodes and applying `docker-compose.gpu.yml` itself into both llama.cpp bring-ups when present (override: `INFOCHAT_LLAMACPP_GPU=on|off`; the decision is printed, never prompted) — the same probe drives the serving class: a GPU host gets `INFOCHAT_LLAMACPP_PARALLEL=3` / `INFOCHAT_LLAMACPP_CTX=32768` plus GPU-class cap writes (`INFOCHAT_LLAMACPP_MEMORY=40g`, `INFOCHAT_LLAMACPP_CPUS=12`) and the GPU-class prompt-cache write `INFOCHAT_LLAMACPP_CACHE_MB=16384` (§7.8.3 sizing arithmetic), a CPU host gets `1`/`4096` and the base cap defaults with no cache write (the base 8192 render), all printed never prompted (§7.8.3); a GPU-class host is additionally offered an optional speculative-decoding draft head (§7.8.3): the operator supplies a download URL or an absolute local path plus an optional SHA-256, with the spec type defaulting to `draft-mtp` and the draft depth to 4 — no head is shipped or pinned (bare Enter declines and writes nothing), and CPU-class, Ollama, and remote hosts are never offered; an accepted head is fetched/staged into the model volume and all five `INFOCHAT_LLAMACPP_SPEC_*` keys are minted into `secrets.env` before the generative bring-up; then wire embeddings to the operator-chosen backend — a second `llamacpp-embeddings` instance (nomic GGUF, `--embeddings`) or the co-running Ollama nomic embedder — never the generative GGUF, dimension 768. **Remote:** collect base-URL + key only. All backends then prompt for chat/summarizer `timeout-ms` + `max-tokens` with backend/GPU/profile-sized recommended defaults (F-live-5; a GPU-class llamacpp host gets the 60000/600 + 60000/400 benchmark timing; `--defaults` takes them unchanged), and CPU-class local runs also scale the five ingest-role `timeout-ms` to the answered chat timeout (the 30s in-app default fits fast backends only). Writes `infochat.llm.*` and `infochat.embeddings.*` (§7.4). | 6 (LLM config) |
...
The llama.cpp container caps and serving shape in `docker-compose.yml` are operator-settable (M1-744/M1-905) — the right values are a property of the operator's model and host, not of the code. Set the keys in the same `--env-file secrets.env` the wizard drives; a deployment that sets nothing renders the defaults — for the cap keys these are exactly the M1-512 starting points:
...
| `INFOCHAT_LLAMACPP_PARALLEL` | `1` | generative server slots — maps to `LLAMA_ARG_N_PARALLEL` on the `llamacpp` service (NOT `LLAMA_ARG_PARALLEL`, which llama-server silently ignores). The default pins the acceptance-tested single-slot posture; the image's implicit default is 4 auto slots, never a tested configuration. |
| `INFOCHAT_LLAMACPP_CTX` | `0` | generative context size — maps to `LLAMA_ARG_CTX_SIZE`. `0` is llama-server's own documented default (context loaded from the model). |
| `INFOCHAT_LLAMACPP_CACHE_MB` | `8192` | generative prompt-cache limit in MiB — maps to `LLAMA_ARG_CACHE_RAM` (name live-verified on the `server-vulkan-b9776` image's `--help`). This MiB limit is FIXED — it does NOT scale with ctx (the token limit does); `8192` is the image's own default, so a deployment that sets nothing renders it byte-stable. |
| `INFOCHAT_LLAMACPP_SPEC_TYPE` | `none` | generative speculative-decoding type — maps to `LLAMA_ARG_SPEC_TYPE`; `none` is the pinned off value verified on the `server-vulkan-b9776` image. The operator chooses the implementation; no model-specific head is shipped. |
| `INFOCHAT_LLAMACPP_SPEC_DRAFT_GGUF` | empty | draft-head filename in the shared models volume — maps conditionally to `LLAMA_ARG_SPEC_DRAFT_MODEL=/models/<filename>`. Empty means no draft model is selected, rather than the invalid `/models/` directory path. |
| `INFOCHAT_LLAMACPP_SPEC_N_MAX` | `4` | draft depth — maps to `LLAMA_ARG_SPEC_DRAFT_N_MAX`; inert while the speculative-decoding type is `none`. |

The wizard (§7.7.2 step 4) writes both serving keys itself from its GPU probe: a GPU-class host gets the campaign-measured serving class (`INFOCHAT_LLAMACPP_PARALLEL=3`, `INFOCHAT_LLAMACPP_CTX=32768`) plus GPU-class cap values through the cap keys above (`INFOCHAT_LLAMACPP_MEMORY=40g`, `INFOCHAT_LLAMACPP_CPUS=12` — GTT pages pin to the container cgroup, so the base 7g/3.0 caps OOM a GPU model class); a CPU-class llamacpp host gets `1`/`4096` with the base cap defaults, and remote/ollama hosts get no llama.cpp writes. The GPU class additionally writes `INFOCHAT_LLAMACPP_CACHE_MB=16384`: the prompt cache lives in the container's address space, and the 40g cap minus ~21 GB Q6_K_XL weights, minus KV for the serving shape, minus ~2 GB spec-decode head leaves room for a 16 GiB cache, while the CPU-class 7g cap does not — so the CPU class gets no cache write and keeps the byte-stable `8192` base render (a GPU→CPU re-run clears the stale key). The raised limit's measured effect: under the prod eval backlog the cache LRU-evicted the chat turn's prefix between turns, and at 16384 MiB an identical 11.7k-token chat prompt measured 9.9 s cold → 0.97 s cached (`.agents/memory-local/prod-state-post-upgrade-20260823.md`). The opt-in `docker-compose.gpu.yml` overlay additionally pins `LLAMA_ARG_N_GPU_LAYERS: "999"` unconditionally on both llama.cpp services: the explicit all-layers pin fails loud at model load where llama.cpp's fit-to-device auto offload would degrade silently into partial offload. GPU wiring — the ngl pin included — lives only in the overlay, keeping the base file startable on GPU-less hosts.
...
- **`restore.sh <bundle>`** — run on the FRESH target (Docker + a clean checkout
  ...
  GGUF from an OLDER bundle (one with no persisted URL) fails loud; a GGUF
  staged from a local path at setup also has no persisted URL, so it fails loud
  with the same manual-staging recipe — the wizard discloses this at setup time
  and the source file is the only re-staging source; a speculative-decoding draft
  head is recovered like a custom GGUF from its persisted URL + SHA before the
  generative server starts, and a staged head (empty persisted URL) fails loud
  with the manual-staging recipe; a remote
...
The V84 tag-tree migration (M1-866) is loud by design: it seeds the v2 tree, maps the flat vocabulary by a deterministic zero-LLM lookup, and **raises E4002 on any stored tag name that is neither a seeded tree node nor a key of its hardcoded lookup** — in `tag`, `post.tags`, `source.bootstrap_tags`, or `scope_tag`. `nostr`/`video` are the ruled cases (deliberately unmapped platform/medium names whose disposal the product ruled on); any other legacy name — an operator coinage, an old bootstrap entry, a tagger free-form — fails the same way. A plain restart of a pre-V84 deployment therefore fails: the failed migration rolls back and `restart: unless-stopped` crash-loops the Collector, or the BootstrapLoader node gate ([../spec/deployment.md](../spec/deployment.md) §Bootstrap behavior on startup) fails fast on the deployed runtime file's non-leaf tag names. The cutover is a sequenced stop-first procedure, not a restart. The SAME sequence runs **once per pre-V84 deployment** — the prod instance, the live test instance, any restored clone or host-migration target — each with its own stop-first pass, none skipped. `prod/scripts/tag-tree-cutover.sh` is its executable core: `tag-tree-cutover.sh preflight` inventories every unmapped name across the four DB surfaces plus the runtime file and writes the rulings skeleton, `tag-tree-cutover.sh apply` executes the operator's per-name rulings from that file, `tag-tree-cutover.sh reconcile-file` converts the deployed runtime file's `tags[]` to tree names, `tag-tree-cutover.sh postflight` verifies the end state. The script reads the migration-owner password from `secrets.env` and passes it as `PGPASSWORD` environment only; it targets the compose-published loopback DB by default (`CUTOVER_PGHOST`/`CUTOVER_PGDB`/`CUTOVER_PGUSER`, or wholesale via `CUTOVER_PSQL`). The host needs a **psql client** on `PATH` (`postgresql-client`) — the one host prerequisite beyond Docker + Compose — or `CUTOVER_PSQL` pointed at a container-exec wrapper that runs psql inside the postgres container, e.g.:
...
**Arriving via a routine upgrade?** `upgrade.sh` runs the same preflight automatically after the pull and **before the build/restart** — on every run, no-op-pull redeploys included. On findings it prints the per-surface inventory, names the rulings file, and exits; nothing but the backup and the pull has happened, so there is nothing to roll back. Complete the rulings and run the cutover below, then re-run `upgrade.sh`.
...
3. **Preflight.** `tag-tree-cutover.sh preflight`. The FIRST run is the inventory: it exits 1, lists every unmapped name per surface with counts — the deployment's actual leftovers, not a guess — and writes the rulings skeleton `$RUNTIME_DIR/tag-cutover-map.txt` (only when absent; later runs never clobber an operator-edited file). The file surface is stricter than the DB ones: every runtime-file tag must already name a tree **leaf** (a V84 mapping key in the file is a finding — it must be converted, not passed). The deployed `bootstrap-sources.json` must use the wizard template's one-array-per-line shape, with every `"tags": [...]` span on a single line: the cutover tooling fails loud on a pretty-printed span, and restoring this shape is the fix. The Collector reads either shape.
4. **Rule every name.** Edit `$RUNTIME_DIR/tag-cutover-map.txt`: one ACTIVE line per unknown name that needs a ruling — `name: <tree-leaf>` maps every occurrence onto the tree leaf, `name: drop` removes every occurrence. A map target must be a leaf whose **row already exists** on this database — the identity leaves `ai`/`crypto`/`research` or your own coinages; every other tree leaf's row only exists after the migration, so a ruling onto one is refused with the alternatives named (map to an existing leaf, or drop). The skeleton pre-fills ACTIVE `nostr: drop` / `video: drop` lines for the standing disposal ruling and comments one placeholder per other name that needs a ruling (carrying its per-surface counts and any raw file forms); complete every placeholder — each is an unknown name that needs a ruling. A file tag that is a V84 mapping key needs no line and gets no placeholder: it converts deterministically (the DB side in the V84 migration itself, the file side in step 6's reconcile-file), and a ruling naming one is refused. Rulings keys are the normalized (lower-cased) stored form. There is no catch-all: a name the file does not cover is refused, by design — every disposal is individually reviewed.
5. **Review, then apply.** `tag-tree-cutover.sh apply --dry-run` prints the per-name plan and the per-surface row counts and changes nothing — review it. Then `tag-tree-cutover.sh apply` validates the whole file against the CURRENT inventory — a malformed line, a duplicate, an extra or stale line, an uncovered name, a map target that is no seeded leaf or whose leaf row does not exist yet, or a map ruling for the ruled `nostr`/`video` each refuses with exit 2 naming the line, before anything changes — and then, in one transaction: re-points or removes the `scope_tag` references first (FK order, `ON CONFLICT DO NOTHING`), retires the tag rows, and rewrites the array elements with order-preserving dedup. Per-surface counts print, followed by the consumed rulings classified for the shared file: a line whose name is also a current file-side unresolved name and resolves via a ruling is **kept for `reconcile-file`**; every other apply-consumed line, including the standing `nostr`/`video` drops, is **retired now, before `reconcile-file`**. Retire only the marked `retire now` lines; keep the lines marked for `reconcile-file` active until that step consumes them. Once a consumer has completed, leaving its consumed line active makes that consumer refuse it as stale.
6. **Reconcile the runtime file.** `tag-tree-cutover.sh reconcile-file --dry-run` prints the deterministic classification of every tag in the deployment's runtime `bootstrap-sources.json` (the compose mount at `/app/bootstrap-sources.json`): a normalized tree leaf is kept (written in its normalized form), a V84 mapping key converts to its leaf (`Java` → `software-development`), `nostr`/`video` are dropped (the standing disposal ruling), and every other unmapped name takes its line from the rulings file — review the table, then run `tag-tree-cutover.sh reconcile-file`. It rewrites ONLY the `"tags": [...]` spans (deduped, order-preserved, normalized forms; every other byte of the file — comments, fields, ordering — survives) and prints the ruling lines it consumed: retire those after this step, including the apply lines marked **kept for `reconcile-file`**. File-only non-key lines are consumed here only. A ruling line for a file-surface name that is a V84 mapping key is never used (keys convert deterministically) and refuses as stale — no key line should exist. The rewritten file takes effect at the next Collector boot via the loader's upsert. Remove the lines marked consumed by this step, then `apply` and `reconcile-file` both accept the empty rulings file as clean no-ops; preflight must now print `preflight: clean`.
7. **Migrate = start the Collector.** `apps.sh start` — only the Collector runs Flyway in production ([../spec/deployment.md](../spec/deployment.md) §Topology); it applies the migration set through V84 and the Provider waits on its healthcheck. Never hand-run V84's statements via psql: the schema change would land without a `flyway_schema_history` row and the next boot would re-apply it over existing objects.
8. **Postflight.** `tag-tree-cutover.sh postflight` — GREEN on: history version 84 applied, 9 tops + 53 leaves, the 8 fallback-marked leaves, zero `nostr`/`video` anywhere, every `post.tags` / `source.bootstrap_tags` element naming a tag node, zero `scope_tag` orphans, and every runtime-file tag naming a tree leaf.
9. **Smoke.** `8-verify.sh` per §7.7.2.

**E4002 at the migrate boot = a leftover survived.** The message names it, and Flyway's transactional per-migration rollback leaves the DB at V83 — rule the named leftover in `tag-cutover-map.txt`, apply, re-run `tag-tree-cutover.sh preflight` until clean, and restart. Never reach for a `pg_restore`: the DB was never damaged.
