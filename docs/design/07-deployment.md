...
| 4 | `4-llm.sh` | Branch on the choice. **Ollama:** start the ollama service and `ollama pull` the profile's chat / security / embedding models. **llama.cpp (D49):** fetch the chosen generative GGUF, mint its filename and recovery metadata, and start the `llamacpp` service, probing `/dev/dri` and applying the GPU overlay when present (override: `INFOCHAT_LLAMACPP_GPU=on|off`; the decision is printed, never prompted). The same probe drives the serving class: a GPU host gets `INFOCHAT_LLAMACPP_PARALLEL=3`, `INFOCHAT_LLAMACPP_CTX=24576`, and `INFOCHAT_LLAMACPP_MEMORY=40g` by default; paired operator-provided `INFOCHAT_LLAMACPP_CTX` and `INFOCHAT_LLAMACPP_MEMORY` values are honored, so larger contexts such as 32k remain available when the cap is sized accordingly. GPU-class prompt-cache writes remain as described below; CPU-class, Ollama, and remote hosts keep their existing behavior. | 6 (LLM config) |
...
The wizard (§7.7.2 step 4) writes the serving keys from its GPU probe: a GPU-class host gets `INFOCHAT_LLAMACPP_PARALLEL=3` / `INFOCHAT_LLAMACPP_CTX=24576` and `INFOCHAT_LLAMACPP_MEMORY=40g` by default; explicit `INFOCHAT_LLAMACPP_CTX` and `INFOCHAT_LLAMACPP_MEMORY` values are honored as a paired operator resource choice for larger contexts. The default derives from the M1-918 prompt floor (6,144), the 600-token chat reply budget, and ~1.4k chars/4 headroom, rounded to 8,192 per slot; the prod lesson that 2.7× the biggest real turn per slot is the stopping point remains the sizing reference for larger deployments. GPU-class cap and cache writes remain unchanged: `INFOCHAT_LLAMACPP_CACHE_MB=16384` plus the 40g default fits with ~21 GB Q6_K_XL weights, KV for the default serving shape, and ~2 GB spec-decode head, while the CPU-class 7g cap gets no cache write and keeps the byte-stable `8192` base render. The opt-in `docker-compose.gpu.yml` overlay still pins `LLAMA_ARG_N_GPU_LAYERS: "999"` on both llama.cpp services, keeping GPU wiring out of the base file.
