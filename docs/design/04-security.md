...
5. **Body size cap.** `infochat.fetch.max-body-bytes` (default 5 MiB). The HTTP client streams and aborts the connection if the limit is exceeded — never buffers an unbounded response.
