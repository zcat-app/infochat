# M1 status board

> **Auto-generated by `/tick status`.** Do not hand-edit. Source of truth is the frontmatter of the files under `docs/plan/m1/tick-tickets/`. If this file disagrees with frontmatter, frontmatter wins; re-run `/tick status` to regenerate.

**Last updated:** 2026-09-02

---

## Summary

| Status | Count |
|---|---|
| pending | 11 |
| in-progress | 0 |
| in-review | 0 |
| escalated | 0 |
| done | 170 |
| deferred | 1 |
| abandoned | 6 |
| **total** | **188** |

---

## Runnable now

Tickets where `status: pending` AND every entry in `blocked_by` has `status: done`.

- M1-959 — Re-read width-32 as owner deltas on both legs (complexity: medium, risk: low)
- M1-965 — Make the provenance notice count only admitted grounding (complexity: medium, risk: medium)
- M1-966 — Fit ladder history by rendered estimate before block drop (complexity: medium, risk: medium)
- M1-968 — Measure KB-miss rate and query-class mix (M0) + Brave spike (complexity: low, risk: low)
- M1-973 — Weather structured source: collector pipeline + getWeather tool (complexity: high, risk: medium)
- M1-974 — Bound price_snapshot staleness; clamp captured_at at ingest (complexity: medium, risk: medium)

---

## In flight

| ID | Title | Status | Last review |
|---|---|---|---|

_(none)_

---

## Blocked

Tickets with `status: pending` AND at least one `blocked_by` entry not yet done.

- M1-967 — blocked_by: M1-965 (pending)
- M1-969 — blocked_by: M1-968 (pending)
- M1-970 — blocked_by: M1-969 (pending)
- M1-971 — blocked_by: M1-970 (pending)
- M1-972 — blocked_by: M1-965 (pending), M1-971 (pending)

---

## Escalated (awaiting user resolution)

| ID | Title | Trigger | Date |
|---|---|---|---|

_(none)_

---

## Done

Showing the 10 most recently `done` tickets (full history is git-log-derivable via `git log --grep "^M1-"`).

| ID | Title | Done date | Verdict |
|---|---|---|---|
| M1-964 | Partition lint: catch indirect ambient key bindings | 2026-09-01 | round 2 APPROVE |
| M1-963 | Pin tracker test seeds to the injected Clock | 2026-09-01 | round 1 APPROVE |
| M1-962 | Enforce the partition-insert lint in the Maven build | 2026-09-01 | round 2 APPROVE |
| M1-961 | Extend temporal grammar for digit-less day-scale phrases | 2026-09-01 | round 1 APPROVE |
| M1-960 | Rename fam-replica-restore.sh to replica-restore.sh | 2026-09-01 | round 1 APPROVE |
| M1-958 | Long-stream finalize degrade: enforce the label revert | 2026-09-01 | round 1 APPROVE |
| M1-957 | Arm the eval lane's temporal window + widening guard | 2026-08-31 | round 1 APPROVE |
| M1-956 | Pin the /topic LIKE escaper; share one escapeLike helper | 2026-08-31 | round 1 APPROVE |
| M1-955 | Re-pin the retrieval characterization emission shape | 2026-08-30 | round 1 APPROVE |
| M1-952 | Two-leg baseline record + pre-registered gating rules | 2026-08-30 | round 1 APPROVE |

---

## Deferred

### blocked-on-new-ticket (1)
- M1-911 → M1-919

---

## Abandoned

Tickets decided against — not implemented as this ticket. Terminal: not reopenable via the driver's `reopen`. `abandoned_reason` records why (`decomposed` = split into shipped children; `superseded` = absorbed by another ticket; `obsoleted-by-spec-amend` = a spec change dropped the requirement; `wont-do-infeasible` = evaluated and judged not worth building). See `docs/process/workflow.md` §Status values.

### decomposed (1)
- M1-869 — Amend the spec for the v2 tag-tree taxonomy

### superseded (3)
- M1-860 — Measure news-category tagger coverage on the local model
- M1-861 — Seed validated news categories into the tag vocabulary
- M1-947 — Re-baseline the retrieval eval on the widened set

### wont-do-infeasible (2)
- M1-910 — Rootless IPv6 compose enablement, capability-derived
- M1-926 — Provider DNS remedy: client-compatible resolver on rootless embedded DNS

---

## Dependency graph

ASCII DAG: nodes are ticket IDs (with status in parens), edges are `blocked_by` AND `deferred_on` relationships. Mark runnable tickets with `←`.

```
M1-785 (done)
  └── M1-784 (done)
        ├── M1-786 (done)
        ├── M1-787 (done)
        │     └── M1-786 (done) [see above]
        └── M1-788 (done)
              └── M1-786 (done) [see above]
M1-789 (done)
  └── M1-790 (done)
M1-791 (done)
  └── M1-790 (done) [see above]
M1-792 (done)
M1-793 (done)
M1-794 (done)
M1-795 (done)
M1-796 (done)
M1-797 (done)
M1-799 (done)
  ├── M1-800 (done)
  │     └── M1-803 (done)
  └── M1-801 (done)
        ├── M1-803 (done) [see above]
        └── M1-805 (done)
              └── M1-803 (done) [see above]
M1-802 (done)
  └── M1-803 (done) [see above]
M1-804 (done)
M1-806 (done)
M1-807 (done)
  └── M1-798 (done)
M1-808 (done)
  └── M1-809 (done)
        └── M1-810 (done)
M1-811 (done)
  ├── M1-816 (done)
  └── M1-817 (done)
M1-812 (done)
  └── M1-816 (done) [see above]
M1-813 (done)
M1-814 (done)
M1-815 (done)
M1-818 (done)
M1-819 (done)
  └── M1-821 (done)
        └── M1-822 (done)
M1-820 (done)
M1-823 (done)
  └── M1-824 (done)
        └── M1-825 (done)
M1-826 (done)
  └── M1-827 (done)
M1-828 (done)
M1-829 (done)
M1-830 (done)
  └── M1-833 (done)
M1-831 (done)
  └── M1-833 (done) [see above]
M1-832 (done)
  └── M1-833 (done) [see above]
M1-834 (done)
M1-835 (done)
M1-836 (done)
M1-838 (done)
  ├── M1-839 (done)
  └── M1-840 (done)
M1-841 (done)
  └── M1-842 (done)
        └── M1-843 (done)
M1-844 (done)
  └── M1-845 (done)
        ├── M1-846 (done)
        │     └── M1-849 (done)
        └── M1-848 (done)
              └── M1-849 (done) [see above]
M1-847 (done)
  └── M1-849 (done) [see above]
M1-850 (done)
  └── M1-851 (done)
M1-852 (done)
  └── M1-853 (done)
        └── M1-849 (done) [see above]
M1-854 (done)
M1-855 (done)
M1-856 (done)
  └── M1-858 (done)
M1-857 (done)
  └── M1-858 (done) [see above]
M1-859 (done)
  ├── M1-858 (done) [see above]
  └── M1-862 (done)
        └── M1-863 (done)
M1-860 (abandoned)
  └── M1-861 (abandoned)
M1-864 (done)
  └── M1-866 (done)
        ├── M1-867 (done)
        │     └── M1-869 (abandoned)
        ├── M1-869 (abandoned) [see above]
        ├── M1-877 (done)
        └── M1-880 (done)
M1-865 (done)
  ├── M1-866 (done) [see above]
  ├── M1-867 (done) [see above]
  ├── M1-868 (done)
  │     ├── M1-866 (done) [see above]
  │     └── M1-869 (abandoned) [see above]
  └── M1-869 (abandoned) [see above]
M1-870 (done)
M1-871 (done)
  └── M1-872 (done)
M1-873 (done)
  └── M1-872 (done) [see above]
M1-875 (done)
M1-876 (done)
  └── M1-877 (done) [see above]
M1-878 (done)
M1-879 (done)
M1-881 (done)
M1-882 (done)
  ├── M1-883 (done)
  │     └── M1-884 (done)
  └── M1-884 (done) [see above]
M1-886 (done)
  ├── M1-885 (done)
  └── M1-895 (done)
M1-887 (done)
  └── M1-888 (done)
M1-889 (done)
M1-890 (done)
M1-891 (done)
M1-892 (done)
M1-893 (done)
M1-894 (done)
M1-896 (done)
M1-897 (done)
M1-898 (done)
  └── M1-899 (done)
M1-900 (done)
M1-901 (done)
M1-902 (done)
M1-903 (done)
M1-904 (done)
M1-905 (done)
M1-906 (done)
M1-907 (done)
M1-908 (done)
  └── M1-909 (done)
        └── M1-920 (done)
              └── M1-921 (done)
M1-910 (abandoned)
M1-912 (done)
  └── M1-913 (done)
M1-914 (done)
M1-915 (done)
M1-916 (done)
M1-917 (done)
M1-918 (done)
  ├── M1-921 (done) [see above]
  └── M1-923 (done)
M1-919 (done)
  └── M1-911 (deferred)
M1-924 (done)
  └── M1-926 (abandoned)
M1-925 (done)
  └── M1-926 (abandoned) [see above]
M1-927 (done)
M1-928 (done)
  ├── M1-929 (done)
  │     └── M1-930 (done)
  └── M1-930 (done) [see above]
M1-931 (done)
M1-932 (done)
  └── M1-935 (done)
M1-933 (done)
M1-934 (done)
  ├── M1-935 (done) [see above]
  └── M1-936 (done)
M1-937 (done)
  └── M1-938 (done)
M1-939 (done)
M1-940 (done)
  ├── M1-941 (done)
  └── M1-955 (done)
M1-942 (done)
  └── M1-944 (done)
M1-943 (done)
  └── M1-944 (done) [see above]
M1-945 (done)
  └── M1-946 (done)
        └── M1-947 (abandoned)
M1-948 (done)
  ├── M1-949 (done)
  │     ├── M1-950 (done)
  │     │     └── M1-952 (done)
  │     └── M1-952 (done) [see above]
  └── M1-950 (done) [see above]
M1-951 (done)
  ├── M1-946 (done) [see above]
  └── M1-952 (done) [see above]
M1-953 (done)
M1-954 (done)
  └── M1-952 (done) [see above]
M1-956 (done)
M1-957 (done)
  └── M1-959 (pending) ← runnable
M1-958 (done)
M1-960 (done)
M1-961 (done)
M1-962 (done)
  └── M1-964 (done)
M1-963 (done)
  └── M1-964 (done) [see above]
M1-965 (pending) ← runnable
  ├── M1-967 (pending)
  └── M1-972 (pending)
M1-966 (pending) ← runnable
M1-968 (pending) ← runnable
  └── M1-969 (pending)
        └── M1-970 (pending)
              └── M1-971 (pending)
                    └── M1-972 (pending) [see above]
M1-973 (pending) ← runnable
M1-974 (pending) ← runnable
```
