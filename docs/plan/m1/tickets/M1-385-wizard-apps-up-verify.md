...
acceptance:
  ...
  - "8-verify.sh polls /q/health on the loopback management bind until ready or a timeout, then prints a green/red summary naming any unhealthy component, exiting non-zero on timeout (manual: run against an up stack reaches green — commit-message evidence)."
...
the loopback management bind and prints a green/red summary so a tester sees the
deployment came up. Reaching a green verify via `prod/setup.sh --defaults` is
the end-to-end proof the whole wizard works.

Blocked on the orchestrator (M1-382) and the app compose services (M1-379).
