...
acceptance:
  ...
  - "`LlmRouter.supportedLanguagesFor(LlmProvider)` (currently lines 265–270) — the hardcoded `Set.of(\"en\")` fallback for `OpenAiCompatibleProvider` is replaced with a config-driven lookup. New shape: per provider name `<name>`, the router reads property key `infochat.llm.<name>.languages` (comma-separated ISO 639-1 codes, e.g. `en,cs`). The lookup runs at `buildFromCdi` time during CDI bean construction (the test seam — the dependency-free constructor accepting hand-rolled `List<Entry>` — is unchanged). When the per-provider key is unset, the default value is `Set.of(\"en\")` so existing deployments (which do not configure `infochat.llm.openai-compatible.languages`) preserve today's behavior exactly. The router's priority-3 default-provider resolution stays unchanged — a missing language match in priority 2 still falls through to the configured `infochat.llm.default.provider`."
  - "LlmRouterTest gains exactly two new scenarios extending the existing test class (no new test file; the file is in `files_scope` as a MODIFIED entry): `forTaskTRANSLATORResolvesProviderWithConfiguredCsLanguage` — hand-rolls an `Entry` whose name is `\"openai-compatible\"`, a `ConfigReader.fromMap(Map.of(\"infochat.llm.openai-compatible.languages\", \"en,cs\"))`, and asserts `forTask(TRANSLATOR, \"cs\")` resolves to that entry's provider. `forTaskTRANSLATORWithoutLanguageConfigStillFallsBackToDefaultProvider` — hand-rolls an Entry with no config-set languages; asserts `forTask(TRANSLATOR, \"cs\")` still returns a non-null provider (the priority-3 default-provider branch). The pre-existing scenarios stay green: the production CDI path's `buildFromCdi` is the only branch the language-source change touches, and the test-constructor path injects pre-baked `Entry` lists that already carried capability sets directly. Behaviour-preserving: the production default of `Set.of(\"en\")` when the config key is unset is asserted by a third new scenario `buildFromCdiDefaultsToEnglishOnlyWhenProviderLanguagesConfigUnset` exercising the production constructor path via a CDI-shape minimal harness (or, if that requires `@QuarkusTest`, a unit test against an extracted `supportedLanguagesFor` static helper)."
  ...
  - "TranslationPipelineIT is a `@QuarkusTest` IT (Shape B+ — full pipeline through the InMemoryAdapter) per `docs/process/test-pyramid.md` §Shape B. Scenario `summaryInCsScopeRunsThroughFullTranslationPipeline`: seed a DM scope with `scope_preferences.language = 'cs'`; seed two RAW posts in the past 24h with deterministic UIDs; configure a `@Alternative` LlmProvider that returns a fixed English prose body for the summarizer task and a fixed `\"<cs-translation>\"` body for the translator task; deliver `/summary -w 24h` through the InMemoryAdapter; assert (a) the outbound message body contains `<cs-translation>` (NOT the English prose), (b) the post.body column for each input post is unchanged (the source post body invariance from spec §Translation flow line 205 — \"Source post bodies are never translated\"), (c) the LLM provider's translator-task invocation count == 1 (one call from the pipeline). Scenario `summaryInEnScopeShortCircuitsTranslator`: same setup but `scope_preferences.language = 'en'`; assert outbound contains the sanitized English prose AND the LLM provider's translator-task invocation count == 0."
...
- **Parallel-development collisions.** None expected. T2-G
  (M1-058) merged before this ticket; T2-H (M1-055
  umbrella + subs) is `pending` but touches a disjoint
  surface (asset commands + per-asset config). The only
  shared seam if both run concurrently would be
  `en.properties` — and T2-C.1 does NOT modify
  `en.properties` (translator errors degrade to English text;
  no new bundle keys in this ticket).
