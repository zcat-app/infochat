...
clarity_check:
  ...
  verdict: WARN
  warnings:
    - "SECURITY-FLAG-CONSISTENT: ChatToolDispatcher.dispatch is the LLM tool-call argument validation boundary; security_relevant: false is defensible because the change is a pure reordering that moves the length check earlier with no change to enforced limits, but worth confirming."
  blockers: []
---
