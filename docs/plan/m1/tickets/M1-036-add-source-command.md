...
clarity_check:
  ...
  verdict: WARN
  warnings:
    - "ACCEPTANCE-RUNNABLE item 2: grep `grep -E '\"add-source\"' AddSourceCommandHandler.java` is not constrained to the name() method body; a string constant elsewhere in the file could satisfy the grep without a correct name() implementation. Low-risk given single-purpose file. No action required; noted for awareness."
    - "ACCEPTANCE-RUNNABLE item 15: per-key verify clause uses `<key>` as a template placeholder (`grep -E '^<key>=' en.properties`). The 14 actual keys are enumerated in the bulleted list immediately above, so developer intent is clear. Template form would fail a mechanical parser reading the grep literally. No action required; noted for awareness."
    - "DoD vs Big-picture notes inconsistency on Branch A audit_log: DoD states 'the bot-admin tag-replacement branch writes an audit_log row' (Branch C only). Big-picture notes §MVP audit_log scope states 'Branch A (fresh insert) and Branch C BOTH write audit_log rows.' Acceptance item 13 only verifies Branch C. If Branch A is also intended to write an audit_log row, no acceptance item asserts it. Recommended clarification: decide whether Branch A writes an audit_log row; if yes, add an IT assertion for it."
  blockers: []
---
...
- **MVP audit_log scope.** docs/design/00-mvp.md §2 commits to
  ...
  events in MVP. docs/spec/commands.md §Source management resolves
  the ambiguity: **only Branch C (bot-admin tag replacement on an
  existing row) writes an audit_log row** — the spec is explicit
  that "this is the single path that mutates the global row's tags
  and is audit-logged." Branch A (fresh insert) and Branch B
  (non-admin subscribed-existing) do NOT write audit_log rows. The
  audit surface BROADENS in later tiers (T2-A onboarding, T2-G
  quarantine, etc.); this ticket commits to the MVP-tier audit scope
  per spec.
...
## Implementation outline (M1-036, generated by Plan subagent on 2026-05-18)

### Files to touch (15 of 15)

Production files (9):

- modify: `infochat-ssrf/src/main/java/io/infochat/ssrf/SsrfGuardedHttpClient.java` — append one new public overload `HttpResponse<byte[]> get(URI uri, Map<String, String> extraHeaders)` that mirrors the existing `get(URI)` (same lock/pin/resolveAndValidate/redirect loop + bounded body read), differing only in that `extraHeaders.forEach(reqBuilder::header)` is called on the request builder before `build()`. The existing `get(URI)` becomes a one-line delegator to the new overload with an empty map (preserving its public signature) — OR the new overload is added alongside and `get(URI)` is left unmodified. Decision: leave `get(URI)` unmodified and copy the loop body into the new overload (the ticket forbids modifying the existing overload). Note: existing `get(URI)` returns `HttpResponse<byte[]>` — the new overload returns the same type for consistency (the implementation notes' mention of `HttpResponse<InputStream>` conflicts with the existing surface and is a non-load-bearing suggestion; return type is not pinned by any acceptance grep).
- modify: `infochat-provider/pom.xml` — add a compile-scope dependency on `infochat-ssrf` (project-version-pinned via `${project.version}`).
- create: `infochat-provider/src/main/java/io/infochat/provider/command/AddSourceArgs.java` — value-object/parser for the positional URL + required `--tags` + optional `--type`/`--category`/`--name`. Constructs an `AddSourceArgs` record on success or returns a typed parse failure carrying a bundle-key reference (one of `error.add_source.tags_required` / `error.add_source.unknown_kind` / `error.add_source.unknown_category` / `error.add_source.malformed_url`).
- create: `infochat-provider/src/main/java/io/infochat/provider/source/KindResolver.java` — `@ApplicationScoped` (or `static`); applies the docs/spec/commands.md §Source management closed table in order: explicit `--type` → host-pattern table → RSS auto-detect → AMBIGUOUS. Hosts folded via `IDN.toASCII(host, IDN.ALLOW_UNASSIGNED)` + `Locale.ROOT` lowercase before compare. Returns one of `KIND_RSS | KIND_NOSTR | KIND_BLUESKY | KIND_REDDIT | KIND_YOUTUBE | KIND_ODYSEE | AMBIGUOUS`.
- create: `infochat-provider/src/main/java/io/infochat/provider/source/UrlProbe.java` — `@ApplicationScoped`; injects `SsrfGuardedHttpClient`. Method `ProbeResult probe(URI uri)` issues a range-GET (`Range: bytes=0-0`) via the new `get(URI, Map)` overload, returns a result with status code + detected `Content-Type` header, OR a typed failure carrying one of `error.add_source.url_unreachable | url_blocked_ssrf | url_timeout`. Catches `SsrfPolicyException` (SSRF-blocked) and `IOException`/`InterruptedException` (network/timeout) and maps to the right key.
- create: `infochat-provider/src/main/java/io/infochat/provider/source/SourceUpsertService.java` — `@ApplicationScoped`; injects `DataSource`. Method `UpsertOutcome upsert(actorUserId, scope, kind, identifier, tags, category, displayName, isBotAdmin)` runs ONE transaction: (1) vocab `INSERT INTO tag (name, display, source_origin) ... ON CONFLICT (name) DO NOTHING` for each `--tag` (only on fresh-insert OR Branch C); (2) `INSERT INTO source ... ON CONFLICT (kind, identifier) DO UPDATE SET ... RETURNING id, xmax = 0 AS was_inserted` (Branch A/C UPDATE only writes `bootstrap_tags` when `isBotAdmin` AND the row pre-existed; Branch B leaves `bootstrap_tags` untouched); (3) `INSERT INTO source_subscription (scope_kind, scope_id, source_id, added_by) ... ON CONFLICT DO NOTHING`; (4) Branch A and Branch C: `INSERT INTO audit_log (... 'ADD_SOURCE', 'source', source_id, ...)` inline. Returns `UpsertOutcome.{FRESH_INSERT | SUBSCRIBED_EXISTING | ADMIN_TAGS_REPLACED}` + the resolved `source.id` + the resolved display_name.
- create: `infochat-provider/src/main/java/io/infochat/provider/command/AddSourceCommandHandler.java` — `@ApplicationScoped implements CommandHandler`. `name()` returns `"add-source"`. `handle(ScopeRef, String rawText)` orchestrates: (a) parse via `AddSourceArgs.parse(rawText)`; (b) load actor `users` row (`is_banned`, `is_admin`) by scope's contact id; (c) ban check; (d) permission gate (DM-any-non-banned + group-group-admin-only via `group_membership` lookup for Group scope); (e) kind resolution via `KindResolver`; (f) URL probe via `UrlProbe`; (g) confirm-or-contradict the RSS-path hint against `Content-Type`; (h) call `SourceUpsertService.upsert(...)`; (i) build outbound reply from BundleLoader: Branch A = `reply.add_source.fresh_insert` + ` ` + `reply.add_source.url_visibility_disclosure`, Branch B = `reply.add_source.subscribed_existing`, Branch C = `reply.add_source.admin_tags_replaced`.
- modify: `infochat-provider/src/main/java/io/infochat/provider/messaging/BundleKeys.java` — append 14 `public static final String` constants (one per new key listed in the acceptance per-key block).
- modify: `infochat-provider/src/main/resources/bundles/en.properties` — append 14 key=value entries. The `reply.add_source.url_visibility_disclosure` value MUST contain the literal substring `visible to bot admins`. The `reply.add_source.fresh_insert` value carries an interpolation token for the source name (e.g. `{0}` or `%s`; match the BundleLoader's `String.format` / `MessageFormat` posture if any exists, otherwise pick one and document inline).

Test files (6):

- add: `infochat-provider/src/test/java/io/infochat/provider/command/AddSourceArgsTest.java` (unit)
- add: `infochat-provider/src/test/java/io/infochat/provider/source/KindResolverTest.java` (unit)
- add: `infochat-provider/src/test/java/io/infochat/provider/source/UrlProbeTest.java` (unit; JDK `com.sun.net.httpserver.HttpServer` fixture, mirroring `SsrfGuardedHttpClientTest`)
- add: `infochat-provider/src/test/java/io/infochat/provider/source/SourceUpsertServiceIT.java` (DB-tier `@QuarkusTest`)
- add: `infochat-provider/src/test/java/io/infochat/provider/command/AddSourceCommandHandlerTest.java` (handler-tier `@QuarkusTest` with mocked UrlProbe)
- add: `infochat-provider/src/test/java/io/infochat/provider/command/AddSourceIT.java` (end-to-end via `InMemoryAdapter`; MVP exit criterion §4)

### Tests

- add: `AddSourceArgsTest.java` — covers acceptance item §"AddSourceArgs": (a) missing `--tags` → `error.add_source.tags_required`; (b) empty `--tags=` → same; (c) unknown `--type` → `error.add_source.unknown_kind`; (d) unknown `--category` → `error.add_source.unknown_category`; (e) malformed URL → `error.add_source.malformed_url`.
- add: `KindResolverTest.java` — covers acceptance item §"KindResolver" (10 per-rule @Tests): explicit `--type rss`; explicit `--type RSS` (case-insensitive); `wss://relay.example.com` → nostr; `https://bsky.app/profile/foo` → bluesky; `https://news.bsky.social/feed` → bluesky (subdomain wins over RSS path); `https://reddit.com/r/x/.rss` → reddit (host wins over RSS); `https://example.com/feed.xml` → rss; `https://example.com/about` → AMBIGUOUS; IDN `https://блюски.рф/feed` folded via `IDN.toASCII`.
- add: `UrlProbeTest.java` — covers acceptance item §"UrlProbe": (a) 206/200 + `application/rss+xml` → SUCCESS with detected Content-Type; (b) 4xx/5xx → FAILURE with `error.add_source.url_unreachable`; (c) SsrfGuardedHttpClient rejection → FAILURE with `error.add_source.url_blocked_ssrf`; (d) timeout → FAILURE with `error.add_source.url_timeout`. Spins up a `com.sun.net.httpserver.HttpServer` per test on `127.0.0.1:0` (mirrors `SsrfGuardedHttpClientTest`), uses the same `LoopbackPermitting` IpBlocklist pattern via the `SsrfGuardedHttpClient` package-private constructor.
- add: `SourceUpsertServiceIT.java` — covers acceptance items §"SourceUpsertService", §"Tag-conflict resolution branches", §"Bot-admin tag replacement audit row". Branch A/B/C per-branch assertions inline (fresh-insert writes 1 source row + N tag rows + 1 subscription + 1 audit_log; subscribed-existing leaves bootstrap_tags unchanged; admin-tags-replaced REPLACES bootstrap_tags + writes audit_log). Idempotency assertion: a second `/add-source` for the same URL by the SAME caller writes no second source row, leaves the subscription, does not rewrite `--tags`.
- add: `AddSourceCommandHandlerTest.java` — covers acceptance items §"AddSourceCommandHandler discovered", §"Permission gate", §"URL ambiguous on probe contradiction", §"URL-visibility disclosure presence/absence". Boots `@QuarkusTest`, injects `InboundRouter` + a CDI-replaced `@Alternative` `UrlProbe` mock so the handler's URL-probe path is deterministic. Tests: dispatcher reaches handler exactly once for `/add-source ...`; banned user rejection; group-non-admin rejection; group-admin proceed; ambiguous probe (`text/html` against an `/about` URL) → friendly error; Branch A reply contains the visibility-disclosure substring; Branch B/C replies do NOT contain it.
- add: `AddSourceIT.java` — covers acceptance item §"AddSourceIT" (MVP exit criterion §4 end-to-end): boots with `@TestProfile(inmemory + allow-low-trust)`, spins up a `HttpServer` for the probe URL, calls `adapter.deliverDm("mvp-user-1", "/add-source <probe-url>?... --tags news,tech")`, asserts: exactly one outbound message containing the fresh-insert reply + URL-visibility disclosure; 1 source row with `kind='rss'` + `identifier=<url>`; 1 source_subscription; `bootstrap_tags = {news,tech}`; 2 new `tag` rows.

No test modifications. The "Authorized test changes" body section explicitly states none.

### Cross-cutting concerns

- **Per-(user, scope) isolation (CLAUDE.md §Key conventions).** `source_subscription.scope_id` for DM is the `users.id` UUID, NOT the contact id; the handler MUST resolve the contact id → users.id BEFORE inserting the subscription. The IT must NOT verify subscription ownership by contact id.
- **Plain-text formatting (CLAUDE.md §Key conventions, decision D30).** Every new bundle value MUST be plain text — no `[text](url)` markdown. The acceptance enforces this via a grep against en.properties. The URL-visibility disclosure's mention of `/list-sources --all` MUST stay in inline-code-style backticks (matching existing `help.cmd.*` entries) or remain a bare slash command.
- **Deterministic SQL retrieval; LLM only for ingest evaluation (CLAUDE.md §Key conventions).** No LLM call anywhere in `/add-source`. The probe is HTTP-only; the kind resolver is a pure function; the upsert is SQL. The acceptance enforces "Anthropic / OpenAI client wiring" as `out_of_scope`.
- **No defensive code for impossible scenarios (CLAUDE.md §Engineering rules).** Validation belongs at boundaries (`AddSourceArgs.parse` over the inbound body, `KindResolver` over a parsed URI, `UrlProbe` mapping SSRF/network exceptions). Internal calls between `AddSourceCommandHandler` ↔ `SourceUpsertService` need no null-checks or defensive guards.
- **Audit-before-effect (Invariant 7).** Branch A and Branch C audit_log INSERTs MUST run in the SAME transaction as the source-row write, and BEFORE the COMMIT. The handler's outbound reply is built AFTER the COMMIT from the returned `UpsertOutcome`.
- **Adapter-name absence in CommandHandler SPI.** The frozen `CommandHandler.handle(ScopeRef, String)` does not carry `adapterName`. The handler resolves the user by `contact_id` alone (works in MVP because only inmemory is active); a one-line inline comment in the handler MUST pin this MVP-only assumption so the future SimpleX/Signal multi-adapter ticket sees the seam.
- **Idempotent transaction shape.** Use `INSERT ... ON CONFLICT (kind, identifier) DO UPDATE SET ... RETURNING id, xmax = 0 AS was_inserted` to distinguish fresh-insert vs existing-row reliably in a single statement (no race between SELECT-then-INSERT).

### Implementation order

1. **`SsrfGuardedHttpClient` overload first.** Without the new `get(URI, Map)` method, `UrlProbe` cannot compile. The new overload is a pure additive change; the existing `get(URI)` and every M1-024/025/026 test stays green.
2. **Provider pom.xml + UrlProbe.** Add the `infochat-ssrf` dependency, then create `UrlProbe`. UrlProbeTest exercises the probe against `HttpServer` fixtures and confirms the SSRF guard fires. Compiling order matters: the dependency must land before `UrlProbe.java` references `io.infochat.ssrf.SsrfGuardedHttpClient`.
3. **`AddSourceArgs` + `AddSourceArgsTest`.** Pure parsing; no DB, no SSRF. Smallest dependency footprint, validates the bundle keys for parse-time errors exist (drives the `BundleKeys` + `en.properties` appends).
4. **`KindResolver` + `KindResolverTest`.** Pure-function unit; independent of DB and SSRF. Drives no new bundle keys (KindResolver's only friendly-error path — AMBIGUOUS — surfaces through the handler, not the resolver).
5. **`BundleKeys` append + `en.properties` append.** Both file edits land together so the BundleLoader CI completeness check (`BundleLoaderTest`) stays green at every intermediate commit. Append all 14 keys at once.
6. **`SourceUpsertService` + `SourceUpsertServiceIT`.** The DB layer. IT uses `@QuarkusTest` against DevServices, exercises Branch A/B/C end-to-end including the audit_log assertion. This is the highest-risk piece (concurrent insert race, RETURNING `xmax = 0`); land it before the handler so test failures localize to the upsert, not the handler.
7. **`AddSourceCommandHandler` + `AddSourceCommandHandlerTest`.** The orchestrator. Boots `@QuarkusTest` with an `@Alternative`-injected mock `UrlProbe` so handler-tier tests don't need a network fixture.
8. **`AddSourceIT`.** End-to-end through `InMemoryAdapter` + `HttpServer` probe fixture + DevServices. Last because every prior piece must be green for this to pass.

A wrong order would: (a) try to compile UrlProbe before the SsrfGuardedHttpClient overload → compile fail; (b) add BundleKeys constants without en.properties entries → BundleLoaderTest fail at the very next test run; (c) write the handler before SourceUpsertService → handler tests blocked on missing upsert seam.

### Risks

- **Provider role lacks INSERT/UPDATE on `source` and `tag` (high; deferred-not-blocker).** V6 grants Provider `SELECT` only on `source` and `tag`; V6's own comment says Provider source writes "route through a future handoff path." Design 04-security.md §4.10 confirms: Provider's write-tables are `users`, `groups`, `group_membership`, `source_subscription`, `scope_tag`, `scope_preferences`, etc. — `source` and `tag` are NOT in the Provider's write set. However the current Provider runtime connects as the `infochat` superuser (see `infochat-provider/src/main/resources/application.properties` line 18 and its comment "stays the bootstrap `infochat` superuser until the named-datasource wiring ticket lands"), so the writes succeed today and the IT passes. When the role-hardening ticket lands and Provider switches to `infochat_provider`, every `/add-source` call will fail with permission-denied unless either (a) the GRANT matrix is widened (V12+) or (b) the writes route through a SECURITY DEFINER stored procedure (the design's "future handoff path"). This is NOT a blocker for M1-036 — but the developer must document it in the handler/service Javadoc so the role-hardening ticket sees the dependency. Escalation: **defer** (file a follow-up ticket against the role-hardening session, do not block M1-036).
- **`get(URI, Map)` return type ambiguity (low).** Implementation notes mention `HttpResponse<InputStream>`, but the existing `get(URI)` returns `HttpResponse<byte[]>` (the `BoundedByteArrayResponse` wrapper that completes after the lock is released). Returning `InputStream` would force UrlProbe to manage the body stream and would change the lock lifecycle. Returning `HttpResponse<byte[]>` keeps the lock/pin/body-read parity with `get(URI)` and gives UrlProbe everything it needs (status code + headers; the body is small under `Range: bytes=0-0`). The acceptance pins neither return type. Escalation: **none needed** — pick `HttpResponse<byte[]>` for parity and note the choice in a one-line Javadoc.
- **Adapter-name absence in CommandHandler SPI (medium).** The handler must resolve `contact_id → users.id` for `is_banned`/`is_admin` lookup + `source_subscription.scope_id`. The SPI doesn't carry the adapter name. In MVP only `inmemory` is active so a contact-id-only lookup works; multi-adapter (T3-A SimpleX+Signal) will need either the SPI widened OR a thread-local adapter context. Escalation: **none needed for M1-036** — note in Javadoc that the handler assumes a single active adapter; T3-A will hit and resolve.
- **BundleLoader orphan-key check (low).** The acceptance asserts "BundleLoader's bundle-completeness CI check (M1-035c's commit) continues to pass: every BundleKeys constant has a matching en.properties value AND no orphan en.properties entries exist." Inspection of `BundleLoaderTest.java` shows it only iterates BundleKeys → bundle (forward direction). Orphan en.properties entries (a key with no constant) are NOT detected by the current test. The ticket's acceptance is more strict than what M1-035c's test actually enforces. Adding the orphan check is a test modification of `BundleLoaderTest`, which IS NOT in `files_scope` AND is explicitly part of the M1-035c frozen surface. Escalation: **refine** if the user wants the orphan-direction enforced (would require widening `files_scope` to include `BundleLoaderTest.java`, breaching the T1-E freeze). Otherwise the ticket's mention is aspirational and the forward-only check is what M1-035c shipped; the developer should treat the acceptance as "BundleLoaderTest continues to pass" (i.e. forward completeness) and note the orphan-direction gap in commit message under `Alternatives considered:`.
- **Idempotency + Branch B detection (medium).** The IT requires "a SECOND `/add-source` for the same URL by the SAME caller is idempotent — no second `source` row, the existing subscription row remains, `--tags` are NOT rewritten on the source row." With the `INSERT ... ON CONFLICT DO UPDATE ... RETURNING xmax = 0 AS was_inserted` shape, the `DO UPDATE` clause MUST be a no-op for Branch B (non-admin caller) so `bootstrap_tags` does not get rewritten. The conditional must be expressed in SQL: `DO UPDATE SET bootstrap_tags = CASE WHEN ? /*isBotAdmin*/ THEN EXCLUDED.bootstrap_tags ELSE source.bootstrap_tags END WHERE source.deleted_at IS NULL`. The `RETURNING xmax = 0` distinguishes inserted-vs-updated; the `isBotAdmin` flag distinguishes Branch B (do-nothing UPDATE) from Branch C (real bootstrap_tags rewrite). Escalation: **none** — the SQL shape is solvable; flag for the reviewer.

### Out-of-scope (echoed from ticket)

- any change under `infochat-messaging-adapter/` (SPI + InMemoryAdapter frozen at M1-035a)
- any change to `SsrfGuardedHttpClient.java` EXCEPT the additive `get(URI, Map<String,String> extraHeaders)` overload; modifying/removing the existing `get(URI)` is forbidden; existing Collector RSS-fetcher callers must continue to use `get(URI)` unchanged
- any change to AdapterRegistry, InboundRouter, MessagingStartup, CommandHandler, AutoRegisterService, BundleLoader, HelpCommandHandler, or the M1-035c bundle infrastructure (BundleKeys is the only narrow append-only carve-out)
- any Flyway migration under `infochat-core/src/main/resources/db/migration/` (V12+ is an escalation trigger, not authoring)
- any `/summary` surface (M1-037)
- any `/list-sources`, `/unfollow-source`, `/remove-source`, `/source-enable`, `/source-disable` (T2-B); the active↔disabled↔failed state machine is NOT exercised; fresh inserts write `status='active'`
- any `/follow-tag` / `/unfollow-tag` / `/tag-mode` handler (T2-B); this ticket unions `--tags` into vocab but authors no follow-tag handler
- any group `@mention` dispatch / group-scope intake (T2-F); the permission-rejection branch is tested but no group-membership infrastructure is added
- any invite-gating (D44), slow-start probation filter (D45), `/ban`/`/unban` (D11), chat-mode dispatcher (T2-A / T2-D)
- any TranslationProvider integration / `/lang` (T2-C)
- any LLM output sanitizer integration (M1-037)
- any Anthropic / OpenAI client wiring (no LLM calls in `/add-source`)
- any RSS Fetcher / FetchScheduler change (M1-022 / M1-023); the per-kind tick is the existing pickup mechanism; NO `pg_notify('new_source', ...)` channel added
- any `scope_preferences` / `scope_tag` write
- any `audit_log.action` verb beyond `ADD_SOURCE` (the V5 §2.1.8 closed catalogue is consumed as-is)
- any new `AuditLogWriter` helper class (inline JDBC INSERTs only, matching `BootstrapLoader.insertAuditRow` style)

### Audit coverage

- file accounting — audited (pass) — 9 production + 6 test files = 15, exactly matches `files_budget: 15` and every entry is present in `files_scope`.
- API-surface — audited (pass) — verified `SsrfGuardedHttpClient` has `resolveAndValidate` + `SsrfPolicyException` (the regex `isAllowed|SsrfPolicy|resolveAndCheck` matches `SsrfPolicy*` in any new overload that calls `resolveAndValidate` or throws `SsrfPolicyException`, even though `resolveAndCheck` does not literally exist); verified `CommandHandler` has the cited `name()` + `handle(ScopeRef, String)` signatures; verified `InboundRouter.handleSlash` iterates `Instance<CommandHandler>` so a new `@ApplicationScoped` handler is auto-discovered; verified `InMemoryAdapter.deliverDm` + `sentMessages()` exist as the IT expects; verified `audit_log.target_kind` CHECK admits `'source'` and `audit_log.action` catalogue includes `ADD_SOURCE`; verified `source` and `source_subscription` schema columns match the upsert plan. One acceptance-text quirk (regex names `resolveAndCheck` which doesn't exist, but `SsrfPolicy` carries the match) noted under Risks.
- test-scaffolding — audited (pass) — 6 test files exactly match `test_plan.adds`; no modifications to existing tests; "Authorized test changes" body section correctly states none.
- cross-cutting concerns — audited (pass) — listed seven invariants the developer must keep visible: per-(user, scope) isolation; plain-text formatting; SQL-determinism boundary; defensive-code rule; audit-before-effect; CommandHandler SPI's missing adapter-name; idempotent upsert shape.
- implementation order — audited (pass) — eight ordered steps with rationale; the wrong orderings (UrlProbe before SsrfGuardedHttpClient overload; BundleKeys before en.properties; handler before SourceUpsertService) are named.
- risks — audited (pass) — five risks identified: (1) Provider role lacks `source`/`tag` write GRANTs but runtime uses superuser today — high-impact but deferred-not-blocker; (2) `get(URI, Map)` return-type guidance ambiguity — low; (3) CommandHandler SPI doesn't carry adapter name — medium; (4) BundleLoader orphan-key acceptance is stricter than the M1-035c test enforces — low; (5) idempotency requires a conditional SQL UPDATE to distinguish Branch B from Branch C — medium. Each risk → escalation mapped.

## Round 1 rework

Reviewer verdict: REWORK (round 1, 2026-05-18). One acceptance-coverage gap; every other check PASS; SCOPE-DRIFT / TEST-INTEGRITY / OUT-OF-SCOPE / NEGATIVE-SPACE / SPEC-CONFORMANCE all PASS.

Fix only the named item below, re-run `mvn -B clean verify`, then `/m1-tick review M1-036` for round 2.

1. **Add a Branch C reply test to `AddSourceCommandHandlerTest`** that seeds an existing source row, invokes `/add-source` as a bot-admin caller with new `--tags`, and asserts the resulting outbound reply (a) does NOT contain the literal substring `visible to bot admins` and (b) contains the `admin_tags_replaced` bundle value's literal substring (e.g. `bootstrap tags replaced`). Acceptance item 12 explicitly requires "Branch B / Branch C tests assert the reply does NOT contain it"; only Branch B is currently asserted at the handler reply level (`AddSourceCommandHandlerTest.branchBSubscribedExistingReplyOmitsUrlVisibilityDisclosure`). Use the existing Branch B seeding helper pattern (insertUser + insertSourceRow with `isAdmin=true` on the caller user row, mock the URL probe with `application/rss+xml` Content-Type), then invoke via `adapter.deliverDm` and assert against `adapter.sentMessages().get(0).text()`.
