...
acceptance:
  - >-
    ...
    `/home/infochat/EMBEDDER-MEASUREMENT-RESULTS.md` §1 (decision block)
    and §3 (calibration evidence). This ticket MUST NOT start until that
    scaffold is filled; every item below needs a measured value.
  - >-
    ...
    fatal-fail
  - >-
    All five thresholds carry the values from results §1, each justified
    by the distribution in results §3 — noting two are cosine DISTANCE
    (`infochat.chat.semantic-threshold`,
    `infochat.linking.semantic-threshold`) and three are SIMILARITY
...
Halo box. Instructions: `/home/infochat/EMBEDDER-HANDOFF-PROMPT.md`.
Results land in `/home/infochat/EMBEDDER-MEASUREMENT-RESULTS.md`.
...
**Spec amendment required.** Three of the five thresholds are hardcoded
