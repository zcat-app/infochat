...
acceptance:
  ...
  - "infochat-collector/pom.xml and infochat-provider/pom.xml each have no explicit <version> element inside any <dependency> block (BOM still supplies versions, M1-001 invariant preserved). Verify per file: `awk '/<dependency>/,/<\\/dependency>/' infochat-collector/pom.xml | grep -E '<version>'` AND `awk '/<dependency>/,/<\\/dependency>/' infochat-provider/pom.xml | grep -E '<version>'` each return zero matches. The awk line-range filter scopes grep to <dependency>...</dependency> contents, correctly excluding the mandatory <parent><version> block and any <build><plugins> versions."
