...
clarity_check:
  ...
  verdict: WARN
  warnings:
    - "COMPLEXITY-RISK-CALIBRATED: migration_touch: true is set but risk: low is claimed; project heuristic associates persistence migrations with higher risk. Non-blocking — the change is an additive, non-destructive CREATE OR REPLACE FUNCTION redeclaration, so practical risk is genuinely low."
  blockers: []
---
