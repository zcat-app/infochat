...
verified_stays_green:
  ...
  - test_class: app.zcat.infochat.provider.messaging.InboundRouterIntakeOrderingTest
    rationale: "drives the router with /help inbounds for the M1-044b intake-step splice ordering; the new step 4.5 sweep slots into the same ordering chain but is a no-op when no pending exists, so the existing ordering assertions remain valid"
  - test_class: app.zcat.infochat.provider.messaging.InboundRouterNormalizeTest
    rationale: "exercises the normalize() static helper directly; the new step 4.5 is downstream of normalize and not exercised by this test"
  - test_class: app.zcat.infochat.provider.messaging.InboundRouterContactIdRedactionTest
    rationale: "@Injects InboundRouter and exercises the dispatch path for contact-id redaction in logs; sends no /ban, /invite create, or /invite revoke and registers no pending state — the step 4.5 sweep is a no-op"
complexity: high
risk: high
round_cap: 3
...
acceptance:
  ...
  - "BanCommandHandlerTest scenario: an admin's first /ban returns the confirm prompt and writes NO `users` row, NO `audit_log` row, NO `invite_code` mutation; `ConfirmStateService.peek` shows pending state for `(actor.id, scope, \"ban\")`. Verify: `grep -iE 'void\\s+\\w*banFirstCallReturnsPromptAndWritesNoStateChange\\w*\\s*\\(' BanCommandHandlerTest.java` returns ≥1 match"
  ...
  - "InviteCommandHandlerTest scenario: /invite create --adapter inmemory --open — the FIRST-call path returns the prompt and writes NO `invite_code` row, NO `audit_log` row; `ConfirmStateService.peek` shows pending state under commandName `\"invite:create:open\"`. Verify: `grep -iE 'void\\s+\\w*inviteCreateOpenFirstCallReturnsPromptAndWritesNoStateChange\\w*\\s*\\(' InviteCommandHandlerTest.java` returns ≥1 match"
  ...
  - "InviteCommandHandlerTest scenario: /invite revoke <code> against a PENDING row — the FIRST-call path returns the prompt and writes NO state change (the row STAYS in PENDING); a follow-up `/invite revoke confirm` within the timeout EXECUTES the M1-044c PENDING→REVOKED transition + INVITE_REVOKE audit row. Verify: `grep -iE 'void\\s+\\w*inviteRevokeFirstCallThenConfirmExecutesRevokeTransaction\\w*\\s*\\(' InviteCommandHandlerTest.java` returns ≥1 match"
...
This ticket modifies two pre-existing test files:

- `infochat-provider/src/test/java/app/zcat/infochat/provider/command/BanCommandHandlerTest.java`
  — ADDS new `@Test` scenarios (banFirstCallReturnsPromptAndWritesNoStateChange,
  banConfirmWithinWindowExecutesBanTransaction,
  banConfirmWithoutPendingReturnsNoPending,
  banConfirmAfterTimeoutReturnsNoPending). NO existing scenario is
  renamed or deleted; the M1-044c happy-path scenarios are augmented
