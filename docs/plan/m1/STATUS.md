...
- M1-529 — Remote LLM backend mis-configures embeddings (model left at nomic against a remote endpoint) (complexity: high, risk: medium)
...
M1-527 (done)
M1-528 (done)
M1-529 (pending) ← runnable
M1-530 (pending) ← runnable
M1-531 (pending) ← runnable
