...
| Group | Tickets | Rationale | Size |
|---|---|---|---|
| **T2-A** onboarding / auth | invite-gating + /ban + /unban, slow-start + /vouch + restricted command set, /grant-admin + /revoke-admin | All cite `docs/spec/security.md` §Authorization model + §Invite-code registration + §Slow-start tier + §User ban. Tightly coupled — replaces T1-E's auto-register path with the v1 onboarding flow. | 3 tickets |
| **T2-B** DM commands on entities | /save + /saved + /unsave + saved_post snapshot, source management (/list-sources + /remove-source + /source-enable/disable + status machine), tag preferences (/follow-tag + /unfollow-tag + /tag-mode + scope_tag) | Same surface conventions (`docs/spec/commands.md` §DM commands); each touches its own schema slice. Three small commands that share the parsing+permission scaffolding. | 3 tickets |
