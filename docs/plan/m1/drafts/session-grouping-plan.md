...
| **T2-C** translation | **M1-059** (TranslationProvider impl — LlmTranslationProvider + 24h translation cache + LlmRouter language-capability widening + SummaryCommandHandler pipeline splice) + **M1-060** (`/lang <code>` command + cs.properties bundle + BundleLoader per-scope language lookup with en fallback). Cites `docs/spec/llm.md` §Translation flow + §Pipeline order + §SPI shape (M1-059) and `docs/spec/commands.md` §Conversation control + §Permission model + spec D43 (M1-060). The two tickets are independent at runtime — M1-059 reads `scope_preferences.language` directly; M1-060 is the user-facing mutator that lets that column move off `'en'`. Recommended start order: M1-059 first (load-bearing translation pipeline) so M1-060's `/lang cs` confirmation reply lands against a working backend. The initial "1 ticket" sizing held until the file-count audit ran — the single-ticket shape exceeded `files_budget: 12` (LlmTranslationProvider + TranslationCache + TranslationPipeline + translator prompt resource + LlmRouter widening + SummaryCommandHandler splice + LangCommandHandler + cs.properties + BundleLoader refactor + their tests ≈ 19 files), so T2-C is split along the natural translator-vs-bundle seam. | 2 tickets (M1-059, M1-060) |
