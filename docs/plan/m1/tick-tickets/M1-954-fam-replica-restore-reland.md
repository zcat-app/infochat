...
reproduction: >-
  ...
  `ls scripts/fam-replica-restore.sh` returns "No such file or directory"
...
session at main tip `ff38802a`: `scripts/fam-replica-restore.sh` does not
...
- **Steps in implementation order:**
  1. `ReplicaRestoreWiringTest` RED (workflow §0): all legs against the
     ...
     file is missing (P8).
  2. Author `scripts/fam-replica-restore.sh`: required flags (source
