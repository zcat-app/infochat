...
2. **Bounded self-refine of a lint BLOCKER or outline FAIL** (decision C). See step 2. This is the *only* place `run` rewrites a ticket on its own, and it is tightly bounded. (There is no longer a clarity-subagent FAIL to self-refine — the clarity gate was replaced by the deterministic linter + the developer self-check at [`start.md`](start.md) step 1; a lint BLOCKER is mechanical by construction, which is exactly what makes the bounded self-refine safe.)
...
2. **Start** — invoke [`start.md`](start.md) end to end, with the grounding behavior from policy 1. `start` runs the mechanical linter (step 1a) then the developer self-check (step 1b).
   - **Lint clean/WARN AND self-check clean** → proceed (WARNs are notify-and-continue).
   - **Self-check raised a blocking `AskUserQuestion`** (genuine ambiguity or a wrong-premise the developer found by reading the code) → this is a real design decision only the user can make. **Halt** to that question — it is a hard stop, not something `run` answers for the user.
   - **Lint BLOCKER → bounded self-refine (decision C):**
     - **Eligibility — mechanical blockers ONLY.** A lint BLOCKER is mechanical by construction (`SPEC-REFS-RESOLVABLE` unresolved anchor, `OUT-OF-SCOPE-PRESENT` empty list, `FORWARD-REFERENCE-RESOLVABLE` unfiled load-bearing ID). Self-refine is permitted only when the orchestrator can fix it without changing scope or intent: write a specific `out_of_scope`, correct a mistyped `spec_ref` anchor to the real heading, file the missing skeleton the `blocked_by`/`remediates` ID points at. Anything implying a **structural** resolution — a real missing dependency the ticket should wait on, a spec anchor that resolves to nothing because the section was never written — is **NOT** self-refinable, and `run` must not silently widen `files_scope`/`out_of_scope` beyond the minimal fix (the SKILL.md cross-cutting rule binds here).
     - **Verify before applying.** Ultrathink the blocker. Try to *falsify* the proposed fix ("what would make this the wrong correction?"). Apply it only if it survives.
     - **Cap: 1 self-refine attempt.** Apply the fix to the ticket file (it is still on `main`; no branch exists yet), commit `M1-NNN: refine ticket spec (lint-blocker fix)` on `main`, clear `clarity_check:`, then **re-run `start`**. Re-running `start` re-fires the linter fresh — that independent re-check is the backstop: a fix that did not truly clear the blocker simply BLOCKs again.
     - **Halt conditions:** any non-mechanical blocker, OR a second BLOCKER after one self-refine → stop and surface the situation to the user (for `complexity:high` outline-fail, `escalate` with `reason: outline-fail`; for a stuck lint blocker, describe it and ask how to proceed). `run` does not self-refine twice and does not self-resolve structural blockers.
   - **`complexity:high` outline-fail** (plan-writer returned `OUTLINE FAILED`) → **halt** and `escalate` with `reason: outline-fail`; this is structural (the plan-writer could not produce an implementable outline), not a prose fix.
...
3. **Implement.** The orchestrator writes the diff in normal Edit/Write/Bash calls. For `complexity:high`, read the plan-writer sidecar (`outline_file:`) before touching code. Run `mvn verify` capturing to the fixed log path per the SKILL.md "Capture `mvn verify` output" rule before moving to review.
   - **Surface an incidental finding at discovery, via a blocking `AskUserQuestion`.** When implementation turns up a real defect outside this ticket's acceptance — most often in a file the diff already has open — stop and ask in the turn you find it. State in one or two lines what it is and what each path costs, then offer: **fold in** (requires `escalate → refine` to add the acceptance item first — an extra changed line tracing to no acceptance item is a `SCOPE-DRIFT-CHECK` fail, so this is never free and must not be offered as free), **fix under `text:`** (only when the edit qualifies under [`docs/process/workflow.md`](../../../../docs/process/workflow.md) §Non-ticket commits rule 8 — comment/message text with no executable line change; land it as its own commit after the ticket merges), **file a follow-up ticket**, or **drop it**. Recommend the cheapest path that actually fits the defect.
     - **It must be a popup, not printed prose.** A printed line loses to tool output and gets scrolled past — the same failure that made [`start.md`](start.md) step 0's grounding line a blocking question. This one was observed too (M1-709, 2026-07-30: a stale assertion message reported in prose, invisible to the user, who first learned of it in the closing summary). The call is only cheap while the file is still open; deferring it to the summary removes the choice and reads as ticket-padding.
     - After the answer, **continue implementing**. Only "fold in" changes this ticket's diff; the other three leave it untouched.
...
- **Blocking question, then continue (ask, take the answer, resume the same phase):** an incidental finding surfaced at discovery per step 3. Unlike a hard stop this does not end the run — but it is a real popup, not a printed line, and implementation waits for the answer.
- **Notify-and-continue (log it, keep going):** a lint WARN; negative-space WARN; an applied bounded self-refine of a mechanical lint blocker; round-1 REWORK; redteam out-of-model items.
