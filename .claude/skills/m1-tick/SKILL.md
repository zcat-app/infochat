...
- **Reuse a still-valid green verify before review (inert-diff gate).** Before running the pre-review `mvn verify`, classify the working-tree-vs-fork-point diff. If it touches NO file the suite can catch — no `*.java`, no `pom.xml`, no `src/**/resources/**` (Flyway migrations included) — i.e. a `Dockerfile*` / `docker-compose*.yml` / `.dockerignore` / docs / script-only change, AND a green `target/m1-tick-test-<ID>-r*.log` already exists whose freshness still holds (no testable file newer than the log, no merge/rebase since — the M1-272 "a green verify stays valid while the tree is unchanged" rule, the same evidence `commit.md` step 2 gathers), present a blocking Skip-vs-Run `AskUserQuestion` and **recommend Skip** (reuse that log as this round's log; a re-run would only re-prove an unchanged suite). When any testable file changed, no green log exists, or a merge/rebase intervened, run `mvn verify` as normal — no menu. This is a *reuse* of an already-green full-suite result, not a skip of the gate: §5 "run the full suite" and the anti-fakery posture stay intact (a valid green full-suite log is what gets relied on) and the human makes the call. (M1-379)
...
- **Never skip `mvn verify`** before review *to bypass a red or stale result*, and never silently. A green full-suite log that is still valid for the current tree (no `*.java`/`pom.xml`/`src/**/resources/**` change since it ran, no merge/rebase since — M1-272) MAY be reused via the user-gated inert-diff Skip/Run menu (§M1 workflow rules "Reuse a still-valid green verify before review"); when any testable file changed or no valid green log exists, run it. If the developer claims tests pass without a green log, refuse and re-run.
