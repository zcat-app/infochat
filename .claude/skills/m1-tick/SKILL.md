...
- **Round-N must-shrink (N ≥ 2) is advisory (2026-07-19 cutover).** Growth along **all three** of files-touched, lines added, lines removed simultaneously vs round-(N−1) with no citable mandate → the reviewer WARNs (informational), it does NOT force REWORK or escalate. The round cap is the real bound on non-convergent rework. Canonical text in [`engineering-rules-verbatim.md`](../../../docs/process/engineering-rules-verbatim.md) §8; round handling in [`subcommands/review.md`](subcommands/review.md) step 6.
...
- **Immediate escalation triggers** (skip remaining rounds): reviewer returns `MANUAL`; developer about to touch a path outside `files_scope` (when set) or listed in `out_of_scope` — a genuine SCOPE-DRIFT FAIL (a `files_budget` file-count overage is advisory as of the 2026-07-19 cutover and does NOT require pre-escalation); tests fail in a way that suggests the ticket's premise is wrong; two consecutive test failures with the same root cause.
