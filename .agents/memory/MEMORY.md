...
- [Audit the instrument, not just the diff](audit-the-instrument-not-just-the-diff.md) — a golden set/eval fixture inherits its world's sampling frame; gates measure conformance to the instrument, never its representativeness, so challenge the frame (which instance's mainstream? does the ground-truth density exist?) BEFORE optimizing against it. Cost: the whole M1-930..947 retrieval campaign optimized a tech-instance world while a live general-news instance ran the same code; the challenge came from the user, not the agent.
...
- [Ticket worktrees live in .worktree/](ticket-worktrees-live-in-dot-worktree.md) — `/tick --parallel` worktrees go under the repo-root git-unversioned `.worktree/` dir, one per ticket, named for the id; never the home directory, never /tmp, not the per-harness dirs.
- [A worktree is missing the gitignored analysis](worktree-missing-gitignored-analysis.md) — tick-lint's ANALYSIS-REF-RESOLVABLE BLOCKER in a fresh worktree means the gitignored `tick-analysis/` file isn't there; copy it from the primary before lint/review.
- [tick-analysis canonical store is the primary](tick-analysis-canonical-store-primary.md) — the gitignored analysis store's only durable home is the primary checkout: analysts write there by absolute path, worktree starts copy in for lint, merge/abort rescue primary-absent files before worktree removal. Eight analyses were lost to worktree-local writes before this rule.
...
- [Doc-edit tiering + spec layering](doc-edit-tiering-and-spec-layering.md) — plain docs (incl. measurement records) edit directly with a non-ticket prefix, never through the flow; docs a test parses need the flow; spec wording approval is independent; spec ← design ← measurement is one-way, spec never cites measurements.
- [Campaign harnesses must disclose excluded paths](campaign-harness-must-disclose-excluded-paths.md) — a stand-in for a production path enumerates what it excludes (and a ticket validates each); an unfilled verdict form in a measurement record is an open decision that blocks, not a closed file; grep .bench before writing any analysis brief.
...
- [Sweep deferred-to-lane debts](sweep-deferred-lane-debts.md) — a ticket deferring work to a lane ("the harness extension is the eval lane's own") creates a debt no gate tracks; grep the corpus for deferral markers before the lane's next ticket. Cost: M1-938's window arm went a week unmeasured while its byte-identical scores were quoted as determinism proof.
...
- [A config rewrite is not done until a boot proves it](config-rewrite-needs-boot-verify.md) — health polls answer for the PRE-rewrite process (config is read at boot); restart + re-verify before declaring done. Cost: an 11.5h-undetected boot-fatal rewrite; detector = M1-907's 8-verify freshness leg.
...
- [llama.cpp serving shape must be pinned](llamacpp-serving-shape-must-be-pinned.md) — `LLAMA_ARG_PARALLEL` is silently ignored (only `LLAMA_ARG_N_PARALLEL` works); an unpinned render is auto slots=4 × model-max ctx + 8 GiB prompt cache, never the tested shape; GTT pins to the cgroup, so Vulkan still needs the memory cap raised (26B Q6 class: 40g).
- [Qwen3.8 spec-dec landscape (updated 2026-09-01)](qwen38-specdec-landscape.md) — Vulkan spec-decode benchmarks before llama.cpp PR #27812 are INVALID (#27805 graph-optimizer bug, wrong output at temp 0); DFlash2 merged to staging branch `xsn/dflash2`, not master, with open correctness bugs (#27407/#27408); DFlash2 sidecars exist for dense 27B only; unsloth NOW ships standalone Flash-Next MTP head GGUFs (2026-09-01) but no runtime statement — load-probe, don't believe the card; `--n-cpu-ffn` is discrete-VRAM-only, no-op on UMA.
- [psql multi-statement -c is one implicit transaction](psql-multi-statement-single-transaction.md) — a failing LAST statement silently rolls back the earlier writes; separate `-c` calls or explicit COMMITs, then re-read what you wrote.
- [pg_restore fixtures via docker cp, not stdin](pg-restore-fixture-via-docker-cp.md) — piping the dump over stdin loses the custom-format magic header; `docker cp` in, then restore the in-container path.
- [SimpleX v7 live-drive ground truth](simplex-v7-live-drive-ground-truth.md) — `/_send` takes a JSON array; `--execute-log messages` misses edits and raced replies, so the client chat DB `chat_items` table is the delivered-text ground truth; corpus-grounded prompts drive long turns, free-form DMs draw instant refusals.
